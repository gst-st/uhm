...
def main():
    ...
    # -- second bridge: window of being = window of tolerance ---------------
    print("\n[WOT] Второй мост: окно бытия ↔ окно толерантности (Siegel)  [И]:")
    print("  P≤2/7 (серость/диффузия)  ↔ гипоарузал (туман, отстранённость)")
    print("  2/7<P≤3/7 (окно бытия)    ↔ ОКНО ТОЛЕРАНТНОСТИ (и чувствовать, и")
    print("                              думать, оставаясь собой)")
    print("  P>3/7 (кристалл/жёсткость)↔ гиперарузал (перевозбуждение, ригидность)")
    zone = ("окно" if 2/7 < 0.309 <= 3/7 else "вне окна")
    print(f"  Пользователь: P(v2)=0.309 ⇒ {zone.upper()} толерантности "
          f"(рабочая зона). Регуляция: вниз из тумана — активация; вверх из")
    print(f"  перегруза — замедление (длинный выдох). Совпадает с окном бытия.")
    print("  ⇒ окно бытия получает клиническое имя и приёмы саморегуляции; "
          "\n    порог P — вычислим, зона — читается (psyche.rs zone_gloss).")
