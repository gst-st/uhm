...
{\small\color{plosgray} August 2026}
\vfill
...
\begin{tikzpicture}[font=\small, >=Stealth]
  ...
  \node[font=\scriptsize, text=ember, align=center] at (-2.9,1.45) {the missing side\\ is $-2$: no such step};
...
\begin{tikzpicture}[font=\small, >=Stealth]
  ...
  \draw[->, thick, plosgray!80] (82:1.5) arc (82:218:1.5);
  \draw[->, thick, plosgray!80] (202:1.5) arc (202:338:1.5);
  \draw[->, thick, plosgray!80] (322:1.5) arc (322:436:1.5);
...
\textbf{Move four: why the outside never sees it.} Here is the
theorem this chapter was written for. All the pushes and noises by
...
\textbf{Words need the right eye.} If the colors of
chapter~\ref{sec:qualia} are real structure, the theory must be
able to \emph{name its own states} --- and the names must be
learnable the way children learn them: by showing, not by defining.
This was put to a machine test, and the result draws the boundary
of language with three honest failures (machine checked). Teach a
simple reader to name states by handing it all forty-eight raw
numbers --- it fails: the words are not written in those
coordinates. Hand it everything \emph{except} the seven ring-angles
--- it climbs to $0.78$ of the way and sticks, confused exactly
where the theory says it must be: between the two signs of one
ring's color, and between the colored and the colorless.
\textbf{No instrument sees color through loudnesses} --- not even a
talking one. Hand it the angles naively --- better, $0.89$, still
short: on quiet rings the angle is noise read off a dead wire. The
cure is the fading law of chapter~\ref{sec:qualia} applied as
engineering --- \textbf{no carrier, no color}: count a ring's angle
only while its carrier is alive. With that single rule the simplest
reader names states at $0.96$--$0.98$. The moral for every
dictionary of feelings: words about quality are learnable --- but
only by an eye that owns the rings; list loudnesses to a ringless
listener for as long as you like, and he will stall at the same
honest ceiling.

And the channel of angles is not a free gift --- the corpus
measured its price (machine checked). A learner whose notes carry
angles completes a whole family of feelings from a few shown ones
--- seven angles determine all twenty-one differences --- and after
twenty-one encounters it answers the world rightly $71\%$ of the
time where a perfect-memory table manages $48\%$: the table must be
shown every case; the angled learner completes the pattern. But set
the same learner in a world whose answers carry no angles behind
them, and it falls \emph{behind} the table by forty points,
faithfully completing a structure that is not there. The angle
channel is a commitment, not a gift: it wins exactly in worlds
woven of rings --- the worlds this book is about.
...
\begin{itemize}[leftmargin=1.4em, itemsep=1pt, parsep=0pt]
  ...
  \item \textbf{The carrier} (the ring's strengths) --- the ink
    ...
    other \stT. Reading rule: no carrier, no color --- an angle on
    a dead wire is noise (machine checked).
...
add the blue boxes, chapters~\ref{sec:qualia},
\ref{sec:oldinstruments} and
...
\ref{sec:oldinstruments}: the wheel --- clock, combinatorics,
storyteller & research wing & machine reconstruction reports \\[2pt]
\ref{sec:check}: the four checks; the registry of twenty-three &
reference: the status registry & $\delta_{CP}$; the Fano noise
