...
\centering
\begin{tikzpicture}[font=\small, >=Stealth]
  % river of agreements: streamlines part around the whirl
  \foreach \y/\gap in {-0.6/1.0, -0.3/1.15, 0.3/1.15, 0.6/1.0}{
    \draw[plosblue!40, thick] (-5.6,\y) -- (-\gap,\y);
    \draw[plosblue!40, thick] (\gap,\y) -- (5.6,\y);
  }
  \draw[plosblue!40, thick] (-5.6,0) -- (-1.35,0);
  \draw[plosblue!40, thick] (1.35,0) -- (5.6,0);
  % the whirl: a logarithmic spiral drawing inward
  \draw[plosblue!90, very thick, ->]
    plot[domain=0:14.2, samples=140] ({90+deg(\x)}: {1.05*exp(-0.14*\x)});
  \node[font=\scriptsize\bfseries, text=plosblue!60!black] at (0,1.45) {a ``particle''};
...
\begin{tikzpicture}[font=\small, >=Stealth]
  ...
  \draw[->, thick, leaf!70!black] (-0.65,1.0) .. controls (0.5,0.25) .. (1.9,-0.85);
