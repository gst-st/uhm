...
but derives computable thresholds, emergent spacetime, and 23 numerical
...
the Fano plane $\Fano$. Crucially, \textbf{every} axiom A1--A5 is itself
derived as a theorem: A1 ($\infty$-topos) from T-76+T-186~\status{T}; A2
(Bures metric) from T-187~\status{T} (triple Petz/Uhlmann/SLD-Fisher
characterisation) and T-189~\status{T} (MaxEnt physical recasting); A3
($N = 7$) from Theorem~S and the Bridge T-15 chain via Hurwitz--Adams and
Fano incidence; A4 ($\omega_{0} > 0$) from viability (no dynamics $\Rightarrow$ no
autopoiesis); A5 (Page--Wootters) from T-87~\status{T}. By
\textbf{T-190}~\status{T} (\emph{Axiomatic Closure}), all five axioms are
derivable from the characterising properties of viable holons
$(\text{AP})+(\text{PH})+(\text{QG})+(\text{V})+\text{MaxEnt}$, making UHM
\emph{self-grounding}: \textbf{zero} independent axioms remain. From this
foundation, over \textbf{275 theorems} follow (T-1 through T-281 with
sub-indexed results), including:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Localization of the hard problem: ``why experience?'' $=$ ``why quantum
        mechanics?'' (T-188~\status{T}, Section~\ref{subsec:foundational-closure});
  \item Self-grounding: all axioms are theorems, with zero free parameters
        (T-190~\status{T}, Section~\ref{subsec:foundational-closure});
  \item Convergence of the self-modeling tower $\vp^{(n)} \to \vp^*$ from any
        initial anchor, resolving the circularity of self-reference
        (T-191~\status{T});
  \item Formal closure against the three major external critiques: the
        List / DeBrota objectivism no-go results are answered by the
        \emph{categorical-monistic route} (T-221~\status{T}); the Multi-Resource
        Quantum Theory completeness concern is closed by showing the Lawvere
        fixed point $\rho^* = \varphi(\Gamma)$ is Pareto-optimal against a
        25-monotone resource vector (T-222~\status{T}); and Putnam-triviality
  ...
  \item Thresholds and numerical ceilings derived from mathematics alone, not
        ...
        $\Phi_{\mathrm{th}} = 1$, $D_{\min} = 2$ (T-151~\status{T}),
        $\mathrm{SAD}_{\mathrm{MAX}} = 3$ (T-142~\status{T});
...
The complete formalization (over 275 theorems, T-1 through T-281 including
sub-indexed results, with full proofs) constitutes the UHM theory archive
