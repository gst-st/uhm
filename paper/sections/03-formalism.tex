...
appendices. The complete formalization (over 248 theorems, including
sub-indexed results T-1 through T-248) constitutes the UHM theory archive;
...
$D_{\mathrm{diff}} \geq D_{\min} = 2$ (T-151~\status{T}) is an
...
$\Lind_0$) and $k = 1-R$ is the compression parameter.
...
(ii)~$\rho^*_\Omega = \vp(\Gam)$ --- Lawvere fixed point of the full $\Lind_\Omega$
\emph{with} regeneration (T-96~\status{T}); a non-trivial categorical self-model,
not a dynamical dissipative attractor.
(iii)~$\rho^*_{\mathrm{coupled}}$ --- equilibrium of an embodied holon coupled to an
environment (T-149~\status{T}: $P(\rho^*_{\mathrm{coupled}}) > 2/7$ unconditionally).
...
$\rho^*_{\mathrm{diss}} = I/7$; the categorical fixed point $\rho^*_\Omega = \vp(\Gam)$
emerges from the full non-linear $\Lind_\Omega$ and is resolved iteratively. Starting
from $\vp^{(0)} := \rho^*_{\mathrm{diss}} = I/7$, the sequence
$\vp^{(n+1)} = \vp(\Gam;\vp^{(n)})$ converges exponentially to $\vp^*$ (T-191~\status{T};
...
The operator-level convergence of the full $\vp$-tower ---
$\|\vp^{(n)} - \vp^*\|_{\mathrm{op}} \leq q^n$ with
$q = \kappa_{\max}/(\lambda_{\mathrm{gap}} + \kappa_{\min}) < 1$ --- is proved
as T-191~\status{T} (Section~\ref{subsec:foundational-closure}), resolving any
residual circularity: the definition is iterative, not circular, and the
limit is independent of the initial anchor. The categorical target of the
...
\emph{minimal}. Furthermore, the nontrivial attractor $\rhostar \neq I/7$
is unique in the viable set (T-124c~\status{T}), and perturbations of
order $\varepsilon$ in $\Gam$ produce only $O(\varepsilon)$ perturbations in
all thresholds (T-124d~\status{T}, with crossover width
$\delta P \sim \varepsilon^4$ from $\beta = 1/4$).
