...
The number 7 is not chosen --- it is \emph{derived}. Two complementary
...
\begin{itemize}[leftmargin=2em,nosep]
  ...
  \item \textbf{A3 \status{T}:} the \emph{number} $N = 7$ from Theorem~S
        (functional minimality); the \emph{structure} from the Bridge T-15 chain
        $(\text{AP})+(\text{PH})+(\text{QG})+(\text{V}) \xrightarrow{\text{T-15}} \mathrm{BIBD}(7,3,1) \to \mathrm{PG}(2,2) \to \mathbb O \to G_2$
        (which consumes $N = 7$ from Theorem~S); Hurwitz--Adams closes the
        consistency loop: $\dim\mathrm{Im}(\mathbb O) = 7$.
