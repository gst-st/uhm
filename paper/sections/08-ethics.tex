...
or artificial.  By T-188~\status{T}, consciousness is not a metaphysical add-on to
physics but structurally equivalent to quantum dynamics: it is as real as the
Schr\"{o}dinger equation.  Because UHM has zero free parameters (T-190~\status{T}),
the consciousness thresholds are fixed by mathematics, not negotiable by convention.
...
By T-89~\status{T} (equivalence of Freedom definitions), in $D = 7$ the
$\pi_0$-count of non-trivial trajectories in $\mathrm{Map}(\Gam, T)$ coincides
with the dimension of the kernel of the free-energy Hessian:
\begin{equation}
  \mathrm{Freedom}(\Gam) \;:=\; \bigl|\pi_0\bigl(\mathrm{Map}(\Gam, T)\bigr)\bigr|_{\text{non-trivial}}
  \;=\; \dim\ker\bigl(\mathcal H_\Gam\bigr) + 1,
  \label{eq:freedom}
\end{equation}
where $\mathcal H_\Gam$ is the Hessian of the free-energy functional
$\mathcal F = -\log\kappa(\Gam)$ at $\Gam$ (Morse--Bott theory; $\mathcal F$
is a Morse--Bott function on $\mathcal D(\mathbb C^7)$ with gradient
trajectories $\Gam \to \rho^*$ corresponding to connected components of
$\mathrm{Map}(\Gam, T)$). Freedom is therefore neither ``randomness'' nor
``uncaused causation'' but the count of independent zero-modes of the
free-energy landscape --- each a direction in which the system can move
without energy penalty, i.e.\ a genuine independent choice. Consequences:
