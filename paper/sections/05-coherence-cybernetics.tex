...
reflection falls below $\Rth = 1/3$).  The dynamics thus ``attracts'' toward $P \approx 3/7$ --- the optimal
balance between coherence and flexibility.  The convergence of $\vp(\Gam)$
within each cycle is guaranteed by T-191~\status{T} ($\vp$-tower convergence):
the self-model converges exponentially from any initial anchor, ensuring the
loop is well-defined at every step.
...
The loop converges to a \emph{unique} nontrivial attractor $\rhostar \neq I/7$
(T-124c~\status{T}), which is robust against perturbations: the thresholds
have only $O(\varepsilon)$ sensitivity (T-124d~\status{T}).
