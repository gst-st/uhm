...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Closes all residual mathematical gaps
        (T-210--T-223).} Fourteen theorems close the last gaps of the
        ...
        $\Regen$ (T-222, \S\ref{subsec:theoretical-closures}), and
        finally the Putnam-triviality foreclosure
        (T-223, \S\ref{subsec:theoretical-closures}) --- a seven-lemma
        cascade establishing the three-level ontology L1 (physical
        vehicle) / L2 (intrinsic categorical class $[\Gam_S]_{G_2}$,
        forced by T-190 zero-axiom closure) / L3 (symbolic readout,
        Lerchner-variable), showing that the Putnam multiplicity acts
        on L1$\to$L3 but has zero purchase on L1$\to$L2, so the UHM
        consciousness predicate is alphabetization-invariant;
        non-UHM-compatible alphabetizers are ruled out as physically
        vacuous (Piccinini--Kim); self-alphabetization is intrinsic
        via the reflection operator $R$ (T-96/T-98), categorifying
        the Maturana--Varela enactivist thesis.
