...
\paragraph{The two irreducible inputs.} Beyond these practical limitations lies a
matter of foundational honesty. The shape of the theory is not the absence of
assumptions but their reduction to \emph{exactly two}, each sharpened to its
minimal core. First, \emph{why quantum mechanics}: the operational basis and the
axioms entail one another (T-190~\status{T}), so the single genuinely external
input is (QG) --- that states form a quantum state space; \emph{given} it, the
CPTP dynamics, the octonions, and $N = 7$ are forced, and everything downstream
is a theorem. Quantum-reconstruction programmes could relocate this posit but not
eliminate it (T-188~\status{T}). Second, \emph{why structure is felt}: the
...
\paragraph{The interaction inversion (T-275).} This reversal of the usual arrow
is sharp. Standard physics runs \emph{forces $\to$ particles $\to$ (perhaps)
mind}; UHM runs \emph{coherence $\to G_2 \to$ forces-derived}, with the same
$\Gam$ carrying an intrinsic, experiential aspect. Concretely, the Standard Model
gauge group is an algebraic sub-structure of the coherence symmetry,
$G_2 = \mathrm{Aut}(\mathbb{O}) \supset SU(3)\times SU(2)\times U(1)$
(T-275~\status{T}): the strong, weak, and electromagnetic interactions are not
fundamental inputs but \emph{derived} sub-structures, and what lies ``before''
