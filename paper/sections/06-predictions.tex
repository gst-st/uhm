...
falsification criterion. Because UHM has zero independent axioms
(T-190~\status{T}), there are no free parameters to adjust: falsification
of any L1 prediction invalidates the entire framework, with no escape hatch.
...
6 & Critical purity & $\Pcrit = 2/7$ (T-96, T-151) & \status{T} & No \\
...
14 & Phase coherence & $\rho^*_{ij}(t) \propto e^{-i(E_i - E_j)t}$ for $\Phi \geq 1$ (T-125) & \status{T} & No \\
\midrule
\multicolumn{5}{@{}l}{\textit{VI. Dynamic (phase transitions)}} \\
16 & Ignition dynamics (L1$\to$L2 avalanche) & $T_{\mathrm{ign}} \sim (P - \Pcrit)^{-1}\kappa_0^{-1}$ (T-158) & \status{T} & No \\
...
18 & Ward suppression & Gap fluctuations $\times\, 19/49$ (T-159) & \status{T} & No \\
20 & Yukawa hierarchy & $\varepsilon_{\mathrm{eff}} \approx 0.059$ (T-64, T-216) & \status{C} & No \\
...
admissible gate schedule; the \emph{value} $3$ uses the depth-normalized
$\Pcrit^{(n)}$ formula above (the $n{+}1$ denominator is a normalization
heuristic~\status{C}) and is cross-checked empirically (SYNARC, $500+$
...
\begin{predictionbox}[Prediction 6: Critical purity (T-96, T-151) \status{T}]
\textbf{Formal statement.}
...
+ T-151~\status{T} ($D_{\min} = 2$ from $\Phi$-threshold), with the phase-transition
classification at $\Pcrit$ given by T-160~\status{T} (swallowtail $A_4$).
...
\noindent
Predictions 1, 10, 12 are L1 tests; predictions 6, 7, 17, 23 are L2 tests; predictions 21, 22
