...
\emph{Lorentzian signature.} Two logically separate steps. \textbf{(i) The
$(1,3)$-split}~\status{T}: the Page--Wootters constraint (T-87~\status{T})
singles out exactly one timelike direction (the PW time \emph{axis} --- not to
be confused with the seven tick states of the clock register), while the
spatial sector is the three-dimensional $\Sigma^3$ of T-120b.
\textbf{(ii) The relative sign} $g_{aa} < 0$~\status{T at reflection
positivity}: it follows from requiring the PW generator to be bounded below
(Osterwalder--Schrader reflection positivity across the PW slice), not from an
ansatz. The KO-dimension-6 real structure $J$ ($J^2 = +1$, $J\chi = -\chi J$,
T-53~\status{T}) fixes the even/odd \emph{grading} --- excluding the Euclidean
$J$-type of KO-dim 0/4~\cite{barrett2007} --- but does not by itself fix the
metric signature; the rigorous realization is the Krein spectral triple with
fundamental symmetry $\beta = \gamma^0 \otimes 1$, in which $D$ is
Krein-self-adjoint and the signature $(+1,-1,-1,-1)$ is read off the
$\beta$-inner product. $\square$
...
\emph{transitive} on the unit sphere $S^5 \subset \mathbb{C}^3$ (fundamental
representation), with isotropy group $\mathrm{SU}(2)$, so
$\mathrm{SU}(3)/\mathrm{SU}(2) \cong S^5$; the vacuum $3$-manifold $\Sigma^3$
is the fixed-radius orbit of the residual $\mathrm{SO}(4)$ acting on the
spatial section (it arises from the constant-curvature homogeneous structure,
not directly as an $\mathrm{SU}(3)$-orbit). The induced metric inherits a
