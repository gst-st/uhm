...
## Механизм: качество как $G_2$-инвариант {#механизм-качества}
...
### Теорема (пространство качества) [Т] (T-300) {#теорема-пространство-качества}

Два состояния структурно неразличимы **тогда и только тогда**, когда связаны автоморфизмом структуры. По $G_2$-жёсткости (T-42a) эта группа есть $G_2=\operatorname{Aut}(\mathbb O)$, а **не** $\mathrm U(7)$. Значит пространство качеств — фактор $\mathcal D(\mathbb C^7)/G_2$.

Счёт размерностей решает всё — именно здесь квалиа либо существуют, либо схлопываются:

| объект | размерность |
|---|---|
| $\mathcal D(\mathbb C^7)$ | $48$ |
| спектр (интенсивность) | $6$ |
| изоспектральное многообразие (интенсивность фиксирована) | $42$ |
| общая орбита $G_2$ (чистая калибровка) | $\mathbf{14}$ |
| инвариантное содержание | $48-14=\mathbf{34}$ |
| **качество при фиксированной интенсивности** | $42-14=\mathbf{28}$ |

Машинно: на 200 случайных состояниях размерность орбиты $\min=\max=14$ — общий стабилизатор дискретен. Будь симметрия $\mathrm U(7)$, орбита через общее изоспектральное состояние была бы $42$-мерной, а фактор — **нулевым**: качество свелось бы к интенсивности, и объяснять было бы нечего. *Квалиа существуют в УГМ именно потому, что $G_2\subsetneq\mathrm U(7)$ — потому что таблица умножения жёстка.*

В максимально смешанном состоянии орбита нульмерна (машинно ровно $0$): у $I/7$ качества нет вовсе, есть лишь голый факт бытия. Качество — это то, что несёт отклонение от равномерности.
...
**(а) $\varphi$-вектор несут ИСКЛЮЧИТЕЛЬНО фазы.** На бесфазовом (вещественном) состоянии $A=0$, значит $v\equiv 0$ тождественно, тогда как симметричное качество остаётся (машинно: $|v|=0$, $|A|=0$ при $|S_0|=0{,}3446$ на случайном вещественном состоянии; на случайном комплексном $|v|=0{,}3797$). Семимерный канал качества живёт *только* в фазах когерентностей — и потому распадается со скоростью декогеренции $5\gamma/21$ (T-59). Это и есть точный объект за интуицией «декодирования семимерных когерентностей»: настоящий семикомпонентный вектор, получаемый из фаз таблицей умножения Фано.
...
:::warning Следствие: потолок самоописания — 34, а не 48 [Т]
Четырнадцать из сорока восьми параметров $\Gamma$ — чистая $G_2$-калибровка: никакая структуросохраняющая операция, а значит и никакой прибор, не различит состояния, отличающиеся только вдоль них. Честный потолок идеального самоописания — **34** числа ($6$ спектральных $+\ 28$ качественных), а не $48$. Всякая цифра полноты, названная ранее против $48$, подлежит пересчёту против $34$ — причём числитель обязан быть выведен тем же прибором заново, а не просто перемасштабирован.
:::
