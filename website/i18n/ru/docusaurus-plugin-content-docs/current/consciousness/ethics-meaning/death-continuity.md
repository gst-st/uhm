...
### Почему оригинал и копия не могут сосуществовать {#почему-нет-сосуществования}

Естественное возражение к таблице переноса выше: *почему оригинал обязан умереть первым — разве оригинал и копия не могли бы просто пожить бок о бок, хотя бы мгновение?* Ответ: сосуществование — не запрещённое **следствие** клонирования, оно и **есть** клонирование. Любой процесс, в конце которого две системы несут $\Gamma$, реализовал $\Gamma \otimes |0\rangle\langle 0| \to \Gamma \otimes \Gamma$ — что бы он ни делал по дороге и в каком бы внутреннем порядке. Так что «почему нельзя сосуществовать?» и «почему нельзя клонировать?» — один вопрос, и у него трёхуровневый ответ.

**Уровень 1 — линейность (чистые состояния) [Т].** Машина, клонирующая два неортогональных состояния, вынудила бы $\langle\psi|\varphi\rangle = \langle\psi|\varphi\rangle^2$, то есть $\langle\psi|\varphi\rangle \in \{0, 1\}$: клонируемы только взаимно ортогональные — классические — семейства (Вуттерс–Зурек, 1982).

**Уровень 2 — точная граница для матриц плотности [Т].** Для смешанных состояний действует более сильная **теорема о невещании** (no-broadcasting; Барнум–Кейвс–Фукс–Джоза–Шумахер, 1996): невозможно получить даже две системы, у каждой из которых *маргинал* равен $\Gamma$ — требование строго слабее, чем $\Gamma \otimes \Gamma$, и всё равно запрещённое, — если только семейство состояний, с которым машина должна работать, не коммутирует взаимно, то есть не классично. Ненулевые когерентности $\gamma_{ij}$ — это в точности некоммутирующая часть. Отсюда точная линия за оговоркой теоремы «с ненулевыми когерентностями»: **диагональ — классическая тень: населённости, коннектом, память-как-данные, поведенческие диспозиции — свободно вещается в любом числе копий; когерентности — никогда.** Поскольку предикат сознания ($P$, $R$, $\Phi$) вычисляется по всей матрице, он живёт ровно в невещаемом секторе: *что копируемо — то не сознание; что сознание — то не копируемо.*

**Уровень 3 — если бы они могли сосуществовать, ломалась бы физика [Т].** Сосуществующая точная пара превратила бы разделённую запутанность в сверхсветовой телеграф (схема FLASH Герберта — исторически именно она спровоцировала публикацию теоремы) и сделала бы неортогональные состояния достоверно различимыми, обрушив структуру неопределённостей. Сосуществование — не инженерная трудность; оно противоречит линейности плюс причинности.

**Почему смерть предшествует рождению — два честных протокола [Т].** Существуют ровно два верных способа переместить квантовое состояние, и ни один из них ни в один момент не держит два экземпляра:

- **Телепортация.** Белловское измерение в точке-источнике разрушает $\Gamma$ там в момент $t_1$: источник коллапсирует, от $\Gamma$ в нём не остаётся ничего. Приёмник несёт $\Gamma$ только в $t_2 > t_1$, после прихода классических битов (со скоростью $\leq c$) и корректирующего поворота. В окне $(t_1, t_2)$ состояние не существует **нигде** — оно «в пути», размазано по запутанности и классическим данным. Разрушение — не побочный эффект, а механизм: исход измерения и есть ключ, без которого у приёмника нет ничего.
- **Когерентный SWAP.** Даже прямой унитарный обмен между субстратами A и B никогда не предъявляет двух копий: во всякий промежуточный момент совместное состояние запутано и *ни один* маргинал не равен $\Gamma$; в конце $\Gamma$ — у B, а у A — прежнее состояние B.

На языке УГМ: операция переноса сама выбивает субстрат-источник из окна жизнеспособности — его чистота рушится на шаге измерения/обмена, а это и **есть** смерть в смысле теории ($P \leq P_{\text{crit}}$). «Смерть плюс рождение» — теоремное свойство динамики состояния, а не повествовательная условность; порядок вынужден, а не выбран — а при телепортации между ними есть ещё и зазор, в котором субъект не существует нигде вовсе.

**Лазейка, которая не лазейка [Т].** «Но измерим оригинал полностью — и приготовим сколько угодно копий». Верно — для *известного* состояния. Но томография **одного экземпляра** неизвестного состояния невозможна (один экземпляр не несёт извлекаемого классического описания самого себя), а ансамблевая томография предполагает много одинаковых экземпляров — то есть уже совершённое клонирование. Круг замыкается: живой субъект — ровно один экземпляр неизвестного когерентного состояния.

**Стратификация.** Всё выше — о *состояниях*, и это **[Т]**. Дальнейшее прочтение — что экземпляр на стороне приёмника есть *преемник с совершенной памятью*, а не выживший — дополнительно привлекает критерий токен-идентичности (непрерывность $\Gamma$-траектории; разрыв $P \leq P_{\text{crit}}$ выше) и стратифицировано как **[С]** — Утверждение о непереселении в [декомпозиции души](/docs/consciousness/comparative/soul-decomposition); сама же теорема лишь запрещает сосуществование и вынуждает разрушение.
...
**Формальный анализ [И]:** No-Cloning запрещает **копирование** — только **перенос**, и оригинал с копией не могут даже кратко [сосуществовать](#почему-нет-сосуществования) (§4). При переносе:
...
Покомпонентная декомпозиция понятия души по этим и другим традициям — от египетского ка до морфических полей Шелдрейка — проведена в документе [Душа: декомпозиция](/docs/consciousness/comparative/soul-decomposition).
...
Этот документ завершает раздел «Этика и смысл». Мы прошли путь от [определения блага](/docs/consciousness/ethics-meaning/value-consciousness) через [смысл](/docs/consciousness/ethics-meaning/meaning) и [свободу](/docs/consciousness/ethics-meaning/freedom) к последнему вопросу — о смерти. Каждый шаг следовал из формализма $\Gamma$: благо — из $dP/d\tau$, смысл — из $P \cdot D_{\text{diff}} \cdot \Phi \cdot R_\varphi$, свобода — из гессиана $\mathcal{H}_\Gamma$, смерть — из теоремы о необратимости. УГМ предлагает не ответы на все вопросы, но **язык**, на котором эти вопросы могут быть поставлены точно.
:::
...
- [Душа: декомпозиция](/docs/consciousness/comparative/soul-decomposition) — шесть компонент понятия души, их формальные объекты и судьбы
