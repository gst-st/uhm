...
**Что берёт УГМ:** Благо — не мгновенное состояние, а **устойчивый процесс** ($dP/d\tau > 0$ — скорость, не точка). Eudaimonia $\approx$ пиковый потенциал $P \cdot D_{\text{diff}} \cdot \Phi \cdot R_\varphi$ (см. [Смысл](/docs/consciousness/ethics-meaning/meaning#пиковый-потенциал)).
...
| 4 | **Когнитивные** | $R_\varphi\uparrow$, $D_{\text{diff}}\uparrow$, $\gamma_{LE}\uparrow$ | L2 | Долгосрочная | Обучение, познание, навыки |
...
**Уровень 4 — Когнитивные.** Рост $R_\varphi$ (качества самомодели) и $D_{\text{diff}}$ (богатства состояний). Обучение буквально увеличивает размерность доступного пространства состояний.
...
## Часть IV. Моральная теория {#часть-iv-моральная-теория}
...
**Динамика само-коррекции [С].** Выравнивание — не только статический оптимум (min Gap ⇔ max $P$), но *тренируемая траектория* с доказанной скоростью и доказанной ценой. Перестройка самомодели $\varphi$ к выравниванию стоит длины пути в пространстве состояний ([полосной закон T-250](/docs/proofs/categorical/formalization-phi#теорема-полосы-rφ)), а устойчивая практика поднимает **базовое** качество самомодели по [закону экспоненциального насыщения тренировки](/docs/proofs/categorical/formalization-phi#механизмы-rφ) $R_\varphi^{\text{base}}(t) = 1 - k^2 e^{-4\eta t}\lVert\bar\Gamma - \rho_\theta(0)\rVert_F^2/P$ (T-249–T-252). Это формальный аналог того, почему добродетель — **практика**, а не переключатель: благо достижимо, с познаваемой скоростью и за познаваемую цену.

---

## Часть V. Разрешение этических дилемм {#разрешение-дилемм}
...
| **Аристотель** | Eudaimonia (расцвет) | $\max_\tau [P \cdot D_{\text{diff}} \cdot \Phi \cdot R_\varphi]$ | Формальная аналогия |
