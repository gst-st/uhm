...
| качество самомодели $R_\varphi$ | $1 - \lVert\Gamma - \varphi(\Gamma)\rVert_F^2 / \lVert\Gamma\rVert_F^2$ — мастер-определение в [Аксиоме септичности](/docs/core/foundations/axiom-septicity) | $[0,\ 1]$ | рабочий $R_{\varphi,\text{th}} = 1/3$ **[И]** | феноменология состояний (сон, медитация, психоделики, патология); функционал смысла |
...
Измерено (`rphi_baseline.rs`): **ленивый прогнозист**, который каждый день
...
$\mathrm{SAD_{MAX}} = 3$ испарился бы. Машинная демонстрация:
`core/examples/ladder_audit.rs` `[О]`.
:::
...
назвать, какая это половина и какую долю массы она покрывает. Машина:
`core/src/couple.rs`; пар двадцать одна по вопросу в день, поэтому первое
