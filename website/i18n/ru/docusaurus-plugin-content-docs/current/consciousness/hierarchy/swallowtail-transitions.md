...
**Почему $\phi^6$, не $D_4$.** При codim 3 в двух переменных $D_4^{\pm}$ катастрофы (эллиптические/гиперболические umbilics) конкурируют с $A_4$. Они различимы симметрией: $D_4$ требует $V(x,y) = x^3 \pm xy^2 + \ldots$ без общей $\mathbb Z_2$, тогда как $\phi^6$ допускает $V_{\mathrm{red}}(-m) = V_{\mathrm{red}}(m)$ (только чётные степени). УГМ-параметр порядка — отклонение от максимально симметричной референции $I/7$; под $\mathrm{shift}_{G_2}$-инволюцией, возникающей из $G_2$-канонической самомодели, $m \to -m$ — симметрия детерминированной динамики. Mather-редуцированный $V_{\mathrm{red}}$ наследует эту $\mathbb Z_2$, ограничиваясь **чётной** $\phi^6$-ветвью и исключая $D_4$.
