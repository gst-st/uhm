...
:::note Заметка о фрейме (после эрратума T-301)
$\mathrm{Gap}(i,j)$ читается в **родном семантическом фрейме** холона: перефазировка осей двигает каждый парный фазовый детектор — это движение есть *почерк* состояния, не его содержание. Калибровочно-инвариантный фазовый объект тройки — её **голономия Фано** $H_p = \arg(\gamma_{ij}\gamma_{jk}\gamma_{ki})$ (см. [Язык качества](/docs/consciousness/phenomenology/qualia-structure#язык-качества)). Внутри фиксированного родного фрейма $\mathrm{Gap}$ остаётся рабочей попарной непрозрачностью, используемой во всей главе.
:::
