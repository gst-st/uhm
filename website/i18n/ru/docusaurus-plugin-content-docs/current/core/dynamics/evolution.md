...
#### Теорема T-262: динамическая трихотомия — $\mathcal{L}_\Omega$ как квантовая GENERIC-система [Т] {#теорема-динамическая-трихотомия}
...
2. **Тепло (Фано-диссипатор).** При произвольных скоростях линий $\{\gamma_p\}$ диссипатор имеет точную форму двойного коммутатора и есть **градиентный поток негэнтропии** $F_D(\Gamma) = D(\Gamma\|\mathbb{1}/7) = \ln 7 - S(\Gamma)$ в транспортной метрике Карлена–Мааса семи линий Фано:
$$
...
Следовательно, мастер-уравнение есть точное разложение **обратимое ⊕ необратимое** в смысле GENERIC: одно киллингово (гамильтоново) поле плюс два градиентных потока, в двух канонических петцевых геометриях (транспорт Карлена–Мааса и Кубо–Мори), ведомых двумя каноническими потенциалами (негэнтропия и относительная энтропия к цели). Условие GENERIC-вырожденности выполняется точно для тепловой пары (унитарный поток сохраняет $S$, значит $F_D$); для потенциала вещества оно выполняется тогда и только тогда, когда $[H_{\text{eff}}, \rho_*] = 0$ (ко-диагональный режим) — иначе член работы переносит потенциал вещества (машинный свидетель неинвариантности: $0{,}087$) [С для пункта о вырожденности].
:::
...
**Машинная верификация.** Двадцать анизотропных некоммутирующих испытаний: тождество двойного коммутатора $1{,}4\cdot10^{-16}$; цепное правило Карлена–Мааса $6{,}5\cdot10^{-15}$; градиентно-потоковое тождество $\|\mathcal{D}[\Gamma] + \mathcal{K}^W_\Gamma(\ln\Gamma)\| \le 1{,}2\cdot10^{-15}$; квадратичная форма EPR $\geq 0{,}42$ вне диагонали, $= 0$ точно на диагональных состояниях, совпадение с $dF_D/dt$ до $5\cdot10^{-7}$ (конечные разности); унитарная изометрия $d_B$ и $S$ до $4\cdot10^{-16}$.
