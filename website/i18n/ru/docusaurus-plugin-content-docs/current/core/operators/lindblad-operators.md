...
- Фано-плоскость несёт таблицу умножения октонионов — точнее: её линии задают, КАКИЕ тройки перемножаются, а знаки задаёт **ориентация** каждой линии. Из $2^7 = 128$ ориентаций настоящую алгебру с делением дают ровно **16** (машина: `octonion_check.rs`; согласуется с классическим счётом $480 = 30 \times 16$ по всем размеченным плоскостям Фано). Все 16 дают изоморфные копии $\mathbb{O}$, поэтому выбор есть калибровка разметки, а не добавочная структура, — но одна плоскость знаков не определяет
