...
2. **Категориальный (κ₀):** Формула $\kappa_0 = \omega_0 \cdot |\gamma_{OE}| \cdot |\gamma_{OU}| / \gamma_{OO}$ ([Т при кинетике первого порядка], [вывод](/docs/core/foundations/axiom-septicity#вывод-kappa0-cycle-flux); Тм. 15.3.1) требует $\mathrm{End}(O)$, $\mathrm{Hom}(O, E)$, $\mathrm{Hom}(O, U)$. При удалении O: κ₀ не определён, дуальность $(\mathcal{D}_\Omega, \mathcal{R})$ теряет структуру.
...
Формула **не постулируется**, а выводится из рамки Ченцова–Петца: Бюрес — **Петц-минимальная** метрика, совместимая с квантовой статистикой (классическая единственность по Ченцову; квантовый отбор экстремальностью внутри семейства Петца). Подробнее: [категориальный вывод κ₀](../foundations/axiom-septicity#структурный-анзац-kappa0).
