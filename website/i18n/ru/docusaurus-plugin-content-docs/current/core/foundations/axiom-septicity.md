...
и именно это вычисляет реализация (`gamma_engine.rs`: `p > P_CRIT && r >= R_THRESHOLD && phi >= PHI_THRESHOLD`). Это не косметический выбор, и именно он делает верной интервальную запись, используемую по всему корпусу:
