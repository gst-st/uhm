...
:::note Принимаемая модель
Раздел 4 предполагает **Gap-лагранжиан** со скалярным полем $\Gamma \in D(\mathbb{C}^7)$ и потенциалом:
$$
V_{\text{Gap}} = \mu^2 \|\Gamma\|^2 + \lambda_3 \sum_{\text{Fano}} \gamma_{ij}\gamma_{jk}\gamma_{ki} + \lambda_4 \sum_{i<j} |\gamma_{ij}|^4
$$
с параметрами $(\mu^2, \lambda_3, \lambda_4)$ как единственными независимыми связями ($G_2$-симметрия запрещает другие). Именно это поле теории и называется здесь «принятой моделью».
:::
...
Трёхпетлевая коррекция к ВФ-точке $\sim 2\%$ — малая, но не пренебрежимая. (С $\lambda_4^*=4\pi^2/63\approx0.627$ и $8\pi^2\approx78.96$ коррекция составляет $\sim2\%$.) Коэффициент $C_1$ — модельно-зависимая паттерн-оценка (см. оговорку о порядке ниже), так что неподвижная точка устойчива с точностью $\sim$ несколько процентов, статус **[Т при модельном допущении]**.
:::
...
$$
\lambda_4(\omega) \to \lambda_4^* = \frac{4\pi^2}{63} \approx 0.627 \quad \text{при } \omega \ll \omega_{\text{Planck}}
$$
