...
Экспериментальное значение: $M_H^{\text{exp}} = 125.20 \pm 0.11$ ГэВ (PDG 2024). В Стандартной модели $M_H$ — свободный параметр. В некоммутативной геометрии (NCG) Чамседдина-Конна масса Хиггса **вычисляется** из спектральной тройки. Вопрос: может ли УГМ сделать то же самое?
