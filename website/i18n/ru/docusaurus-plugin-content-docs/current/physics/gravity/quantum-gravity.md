...
- $f_4 = \int_0^\infty u \, e^{-u} \, du = \Gamma(2) = 1! = 1$ (прежнее «$3!=6$» ошибочно отождествляло это с $\int_0^\infty u^3 e^{-u}\,du$; здесь $\int_0^\infty u^{n}e^{-u}\,du = n!$, так что $f_{2m}$ использует $n=m-1$). Ниже последовательно используется соглашение о моментах спектрального действия $(f_0,f_2,f_4)=(\text{рег},1,1)$.
