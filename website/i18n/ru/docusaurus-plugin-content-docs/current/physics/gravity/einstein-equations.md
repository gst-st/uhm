...
Линеаризованный (Фирца–Паули) вывод ниже даёт $G_N^{(ST)}=4\pi\mu^2/\varepsilon_0^2$ ($G\propto\mu^2$), согласуясь с формой выше. **Первичная** ньютоновская постоянная — результат спектрального действия $G_N = 3\pi/(7 f_2\Lambda^2)$ с обрезанием $\Lambda\sim M_P$ (§ниже, T-65 [T]); формула эмерджентной гравитации через $\mu$ — эффективное соотношение, в котором $\mu$ есть **планковский** Gap-параметр ($\mu_{\text{phys}}=\mu\,\omega_0\sim M_P$), а **не** масштаб $\sim$мэВ, используемый в $\Lambda$-секторе. Обе тогда согласны, что $G_N^{-1/2}\sim M_P$.
:::
...
**Машинная верификация** (`t263_t264_learning_gravity_oracle.py`): тождество сбалансированного блока до $6{,}7 \cdot 10^{-16}$; тождество развязанной пары в $\mathbb{C}^7$ — отношение $1{,}000000$ на случайных населённостях/когерентностях; измерено масштабирование перекрёстной поправки ($0{,}23\%/0{,}8\%/4{,}4\%$ при $\bar\varepsilon = 0{,}005/0{,}01/0{,}02$); подстановочная цепочка (b) точна.
