...
Ранее это было помечено как «мост» между двумя объектами. Аудит показывает, что дело серьёзнее: шаг нельзя починить как записано, по двум независимым причинам. Обе проверены машиной (`internal/vanchurin-bridge/spacetime_audit.py` §A–§B).
...
**Вычисление, целиком из октонионов** (`internal/vanchurin-bridge/spacetime_audit.py` §G):
