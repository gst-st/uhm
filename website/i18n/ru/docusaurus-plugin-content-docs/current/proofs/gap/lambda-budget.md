...
**Статус [Т при T-64] через T-219 (замена 2026-04-17)**: прежнее разложение «14 → 7_лёгких ⊕ 7_тяжёлых» G₂-adjoint было **математически невалидно** — $\mathrm{adj}(G_2) = \mathbf{14}$ неприводимо под $G_2$ и не допускает такого расщепления. **T-219** [Т при T-64] ([Фундаментальные замыкания §13](/docs/proofs/categorical/fundamental-closures#t-219)) заменяет это строгим выводом:
