...
Отождествление атомов классификатора Ω с проекторами |i⟩⟨i| — **конструктивное определение**, согласованное с аксиоматикой, а не вывод из абстрактной теории ∞-топосов. Обоснование: (1) в D(ℂ⁷) минимальные нетривиальные подобъекты — ранг-1 проекторы; (2) Бюрес-топология (A2) выделяет их как атомы J_{Bures}-покрытий; (3) результат согласован с L-унификацией ([Т]) и Фано-структурой ([Т]). Формальный вывод из аксиом Лури для Sh_∞(𝒞) — [П] (открытая программа).
:::
...
:::warning Статус доказательства
Доказательство эквивалентности представлено как **скетч** — полный вывод требует проверки трёх нетривиальных шагов: (a) PW↔Бюрес: равномерность дуги Бюрес для PW-потока; (b) Бюрес↔категорное: геодезики Бюрес = минимальные 1-морфизмы в Exp_∞; (c) PW↔стратификационное: число PW-тиков = глубина огрубления. Шаг (a) доказан (стандартный результат для дискретного потока). Шаг (b) доказан при условии, что Exp_∞ наследует Бюрес-метрику через F ([Т], §5 categorical-formalism). Шаг (c) — [С] (зависит от конкретного выбора стратификации). **Обновлённый статус: [Т] для (a)+(b), [С] для полной 4-сторонней эквивалентности.**
:::
