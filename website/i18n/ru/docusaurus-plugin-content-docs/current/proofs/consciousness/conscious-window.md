...
$g_d$; при $\omega_0 = 40$ — в пределах $2.3 \cdot 10^{-3}$. Поправка над
воротами монотонна — растёт с $g_d$, падает с $\omega_0$, в быстром пределе
линейна по $g_d$, — но **не** является функцией одного отношения
$g_d/\omega_0$ (равные отношения дают границы, различающиеся в 1.6 раза;
простой закон опровергнут `[✗]`). Ниже пола часов вопрос растворяется: при
