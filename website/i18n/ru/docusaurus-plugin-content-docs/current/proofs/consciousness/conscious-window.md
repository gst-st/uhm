...
с пределом $C \approx 1{,}71$. Сам коэффициент *выведен* `[С]`: это
седловой баланс $C = (P_s - \tfrac17)/(7B(s - P_s))$ с
$B = \tfrac17 + \hat\kappa_0\,\mathrm{Coh}_E$ и $s = \mathrm{tr}(\Gamma\rho^*)$,
измеренными на сепаратрисе ($B \approx 0{,}180$, $s \approx 0{,}353$), — он
воспроизводит прямой замер с точностью до $1\,\%$; открытой `[Г]` остаётся
лишь замкнутая форма седловых величин — и с нею кандидат
$C = 12/7 \approx 1{,}714$. В среднем же
режиме ($\omega_0 \le 40$) медленнее затухающая добавка ломает форму одного
отношения (равные отношения там различаются в 1.6 раза) — закон
асимптотический, не равномерный. Ниже пола часов вопрос растворяется: при
...
`awakening_boundary_law.rs`; реестр: NUMBERS-LEDGER, запись о законе
границы.)*
