...
- $N_{33}^\mathrm{Fano} = 2$ — число не-$O$ прямых Фано, пересекающих сектор $\mathbf 3 = \{A,S,D\}$ ровно в двух точках, а именно $\{A,S,L\}$ и $\{S,D,E\}$.
  ...
  Прежние редакции полагали $N_{33}^\mathrm{Fano} = 1$ с обоснованием «единственная прямая $\{L,E,U\}$ PG(2,2)». Такой прямой нет. Семь канонических прямых суть $\{A,S,L\}, \{D,L,U\}, \{L,E,O\}, \{A,E,U\}, \{A,D,O\}, \{S,D,E\}, \{S,O,U\}$; прямая через $L$ и $E$ — это $\{L,E,O\}$, а прямая через $E$ и $U$ — это $\{A,E,U\}$. Тройка $\{L,E,U\}$ есть **сектор** $\bar{\mathbf 3}$, а не прямая, и более того **ни одна** прямая Фано не лежит целиком внутри трёхэлементного сектора: прямая, содержащаяся в трёхэлементном множестве, совпадала бы с ним, а ни $\{A,S,D\}$, ни $\{L,E,U\}$ среди семи нет. Ошибка состояла в чтении секторной тройки как прямой. Машинно проверено против канонического набора прямых, удовлетворяющего BIBD(7,3,1): 21 пара, каждая на единственной прямой, каждая точка на трёх.
  :::
...
**Использованные входы** (из численной минимизации T-64): $V_4/V_2 \approx 0.5$ на $\theta^*$; $\Sigma_0 = \sum\theta^{*2} \approx 0.3$; секторальное $|\bar\gamma| \approx 0.15$, глобальное $\bar\varepsilon \approx 0.023$.
