...
- Пертурбативный: $\sim 10^{-41.5}$ [T] (включает $\varepsilon^6$);
- SUSY-секторный $\varepsilon^{12}$ поглощает $\varepsilon^6$: чистое среднее $\to \sim 10^{-53.5}$ [T при T-64 для структуры; С для значения $\varepsilon$];
- Когомологический $\Lambda_\mathrm{global} = 0$: точный нуль [T] — отдельный класс, переформулирует вопрос как размер **локального** остатка;
...
Теорема T-222 доказывает первую альтернативу: **УГМ является MRQT-полной в своей области применимости** (марковская + низкотемпературная). Расширение не требуется. Оптимум характеризуется *спектральной мажоризацией*, а не (пустым) $G_2$-инвариантным подмногообразием — исправленная формулировка ниже.
...
3. **Низкотемпературность** — $\beta H_\text{eff} \ll 1$ (леммы L2 и L6 используют $\rho_\beta \approx I/7$).
