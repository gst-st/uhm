...
**Следствие для T-67.** «Эвристическое разложение 3+1», обозначенное в [стратификации T-67](/docs/consciousness/hierarchy/interiority-hierarchy#теорема-l3-k4), теперь **выводится из трикатегориальной когерентности** (3 клетки — это LGKS-триадные 2-клетки, +1 клетка — модификация когерентности $\eta$). T-67: счёт $K = 4$ имеет статус [T] с полным категориальным обоснованием через T-217.
