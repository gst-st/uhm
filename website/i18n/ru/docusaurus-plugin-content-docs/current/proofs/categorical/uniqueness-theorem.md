...
### Лемма G4: Калибровочная группа октонионной структуры — $G_2$ [Т] {#лемма-g4}

:::tip Лемма G4 (Калибровочная группа октонионной 3-формы) [Т]
Максимальная подгруппа $\mathcal{G} \subseteq U(7)$, сохраняющая октонионную ассоциативную 3-форму $\varphi_3 = \sum_{i<j<k} f_{ijk}\, e^i\wedge e^j\wedge e^k$ — эквивалентно, структурные константы $f_{ijk}$ Леммы G3(ii) — есть в точности $G_2 = \mathrm{Aut}(\mathbb{O})$.

Остальные структуры Леммы G3 — атомарные проекторы (i), E-проекция (iii), PW-часы $O$ (iv), формула $\kappa_0$ (v) — **не** $G_2$-инвариантны; они фиксируют **функциональный репер** (выбор калибровки) внутри каждой $G_2$-орбиты. Два представления, связанные $U\in G_2$, переводят свои реперы друг в друга.
:::

**Доказательство.** Покажем $\mathcal{G} = G_2$ двумя включениями.

**(A) $G_2 \subseteq \mathcal{G}$.** По определению $G_2 = \{g\in GL(7,\mathbb{R}) : g^\ast\varphi_3 = \varphi_3\}$ сохраняет 3-форму, а $G_2\subset SO(7)\subset U(7)$ сохраняет эрмитову структуру. Следовательно, каждый $g\in G_2$ сохраняет $\varphi_3$, т.е. $g\in\mathcal{G}$. $\checkmark$

:::warning Функциональные метки — данные репера, а не $G_2$-инварианты
Поскольку $\mathbb{C}^7$ — **неприводимый** $G_2$-модуль (Cartan 1894), по лемме Шура он **не** имеет ненулевого собственного $G_2$-инвариантного подпространства. Следовательно:
- ни одна координатная ось $|k\rangle$ — в частности оси E, O, U — не $G_2$-инвариантна; генерический $g\in G_2$ её поворачивает;
- *набор* атомарных проекторов $\{|k\rangle\langle k|\}$ сохраняется лишь **конечной** реперной подгруппой $\Gamma_{\!\text{oct}} = \mathrm{Aut}(PG(2,2))\cong PSL(2,7)\subset G_2$ (порядка 168), а не всей $G_2$;
- следовательно, $\mathrm{Coh}_E$, $\Phi$ и $\kappa_0$, которые ссылаются на оси E/O/U, **зависят от репера**: инвариантны относительно $\mathrm{Stab}_{G_2}$ выбранного репера, но не относительно всей $G_2$. Они физичны потому, что репер закреплён динамикой ($\mathcal{L}_\Omega$-ковариантность из Определения G1), а **не** потому, что спускаются на $\mathcal{D}(\mathbb{C}^7)/G_2$.

Подлинно $G_2$-инвариантное содержание — это спектр (6 чисел) плюс углы относительно $\varphi_3$ (28) — те самые $48-14=34$ параметра Следствия 1.
:::
...
Поскольку оба представления удовлетворяют аксиомам A1–A5, унитарный оператор $U$ должен сохранять **октонионную 3-форму** $\varphi_3$, несомую аксиоматической Фано-структурой (Лемма G3(ii) [Т]). Функциональные метки (атомарные проекторы, E-проекция, PW-часы, $\kappa_0$) переносятся ковариантно: $U$ отображает репер представления 1 в репер представления 2.

По Лемме G4 [Т], сохранение $\varphi_3$ даёт $U \in G_2$. $\blacksquare$
...
Необходимо различать два класса наблюдаемых (неприводимость $\mathbf 7$, Лемма G4):

**Подлинно $G_2$-инвариантные** (спускаются на $\mathcal{D}(\mathbb{C}^7)/G_2$):
- Чистота $P = \mathrm{Tr}(\Gamma^2)$ — фактически $U(7)$-инвариант, тем более $G_2$-инвариант
- Мера рефлексии $R = 1/(7P)$ — функция от $P$, следовательно $G_2$-инвариант
- спектр (6) и углы относительно $\varphi_3$ (28) — те самые 34 параметра выше

**Зависящие от репера** (определены лишь после фиксации функционального репера динамикой; инвариантны относительно $\mathrm{Stab}_{G_2}$ репера, но *не* относительно всей $G_2$, поскольку ни одна ось не $G_2$-инвариантна):
- E-когерентность $\mathrm{Coh}_E(\Gamma)$ — ссылается на ось E
- Мера интеграции $\Phi = \sum_{i\neq j}|\gamma_{ij}|^2/\sum_i\gamma_{ii}^2$ — ссылается на координатный базис
- коэффициент регенерации $\kappa_0$ — ссылается на оси O, E, U

Эти зависящие от репера величины физичны потому, что динамика ($\mathcal{L}_\Omega$-ковариантность, Определение G1) закрепляет репер; они не входят в число 34 орбитальных инвариантов.
