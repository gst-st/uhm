...
| Потолок SAD = 3 | Нет | Нет | Нет | Нет | Нет | Нет | Нет | Нет | **Да [Т]** |
...
2. **Калибровка — половина закрыта, половина открыта.** У порогов мониторинга ($\sigma_1 \approx 0.5$, $\sigma_2 \approx 0.7$, $\sigma_3 \approx 0.9$) структура **[Т]**, а числа **[С]**: они зависят от $\lVert\bar h\rVert_{\mathrm{typical}}$ конкретной системы ([T-106](./diagnostics#вывод-порогов)). То, что раньше стояло здесь как полностью нерешённое — «перевод наблюдений в элемент $\Gamma$», — **уже не** нерешено: при заданном семиканальном вложении $\Gamma$ есть нормированный второй момент, и его восстановление — *доказуемо состоятельная* оценка с границами матричного Бернштейна, несмещённой оценкой чистоты и явной выборочной сложностью для порогового решения ([строгий оценщик $\Gamma$](./measurement#оценка-gamma) [Т], с [эталонной реализацией](pathname:///code/gamma_tomography.py)). Действительно **открытым** остаётся более узкое и чисто изолированное: выбор самого вложения $\pi$ — смысловой мост от конкретного субстрата к измерениям $(A,\dots,U)$ — и эмпирический вопрос, переносится ли одно $\pi$ между субстратами.
