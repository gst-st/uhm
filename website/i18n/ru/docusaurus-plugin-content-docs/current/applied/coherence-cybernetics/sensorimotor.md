...
Следовательно, пара знаков $(\operatorname{sign}\dot{S}, \operatorname{sign}\dot{P}) \in \{(0,0),\ (+,-),\ (\mp,\pm)\}$ однозначно определяет действующий канал: трихотомия *наблюдаема*.
:::
...
**Машинная верификация.** Триста случайных полноранговых состояний на канал: $|\dot{S}|, |\dot{P}| \leq 3\cdot10^{-15}$ для $h^{(H)}$; $\min \dot{S} = +0{,}337 \geq 0$ и формула чистоты точна до $10^{-16}$ для $h^{(D)}$; энтропийная формула $h^{(R)}$ точна до $2\cdot10^{-15}$, свидетели обоих знаков — как указано.
...
4. **Дискретность и $U(1)$.** В SLU $U(1)$-фаза волновой функции есть термодинамическая эквивалентность $S \to S + h\Delta N$ при *целочисленных* скачках $N$. Нейрогенез УГМ (⊕-примитив) меняет счёт суб-холонов ровно такими целыми скачками, а диагональная калибровочная группа $U(1)^7 \rtimes G_2$, сохраняющая Фано-проекторы, предлагает поканальную версию того же механизма — одна фаза на канал при одном счётчике заполнения на канал. Записано как интерпретивная параллель [И]; вывод $U(1)^7$ из дискретности заполнения открыт.
5. **Линейно-разрешённые температуры.** Тепловой канал УГМ несёт семь скоростей $\{\gamma_p\}$ — по одной на линию Фано, — тогда как SLU несёт один скаляр $T$. Изотропная точка $\gamma_p = \gamma$, единственная $G_2$-симметричная конфигурация, есть в точности сеттинг SLU: корпус уточняет единую температуру до *спектра семи линейных температур*, где анизотропия меряет, насколько неравномерно среда греет когерентную структуру холона.

Честность статусов: сигнатурная теорема — [Т]; сам словарь — отождествление с (работа, тепло, вещество) и с $(A, T, \mu)$ SLU — интерпретация [И]: точная на сигнатурах и счёте, структурная (ещё не выводная) на соответствии динамических законов.
