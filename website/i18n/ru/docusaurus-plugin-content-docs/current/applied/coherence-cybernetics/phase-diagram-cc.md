...
### 1.3 Точное окно: сколько питания стоит сознание {#точное-окно-питания}

Формула $r_c = P_{\text{crit}}/(7P)$ выше — **размерная эвристика [И]**: полезный порядковый якорь для топологии фаз, но не выведенный из микроскопической динамики. Точное утверждение, полученное прямо из канонической Фано-дефазировки и функтора замещения, — следующая теорема. Её содержание одним предложением: стационарное состояние удерживает лишь *долю* $\lambda = x/(1+x)$ когерентности цели, где $x$ — отношение питания, — и прогон этой удержанной доли через четыре порога сознания вырезает замкнутое окно по $x$.

#### Теорема T-259 (Окно сознания в отношении питания) [Т в изотропной модели первого порядка] {#теорема-окно-питания}

:::tip Утверждение
Изотропная модель: Фано-дефазировка со скоростью распада внедиагоналей $\Gamma_2$, поток замещения $\kappa_{\text{eff}}(\rho^* - \Gamma)$ к равнонаселённой цели $\rho^*$ ($\mathrm{diag} = 1/7$) с чистотой $P^*$ и когерентной массой $C^* = P^* - 1/7$. Пусть $x = \kappa_{\text{eff}}/\Gamma_2$. Тогда:

1. **Стационар.** У $\Gamma_\infty$ точно $\mathrm{diag} = 1/7$, а внедиагонали равны $\lambda\,\gamma^*_{ij}$ с фактором удержания $\lambda = x/(1+x)$; чистота $P_\infty = 1/7 + \lambda^2 C^*$.
2. **Пол (жизнеспособность ≡ интеграция).** $P_\infty > P_{\text{crit}} = 2/7$ (эквивалентно $\Phi_\infty > 1$ на страте) тогда и только тогда, когда
$$
x > x_{\min} = \frac{\lambda_1}{1-\lambda_1}, \qquad \lambda_1 = \frac{1}{\sqrt{7P^* - 1}}.
$$
На потолке Goldilocks $P^* = 3/7$: $x_{\min} = 1 + \sqrt{2} \approx 2{,}414$ (серебряное сечение).
3. **Потолок (рефлексивность).** $R_\infty \geq 1/3$ тогда и только тогда, когда $P_\infty \leq 3/7$. Для сверхчистой цели $P^* > 3/7$ окно двустороннее: $x \leq x_{\max} = \lambda_2/(1-\lambda_2)$ с $\lambda_2 = \sqrt{2}\,\lambda_1$ — **диссипация защищает рефлексивность** от сверхжёсткого идеала (при $P^* = 0{,}60$: окно $x \in (1{,}268;\ 3{,}775]$).
4. **Нежизнеспособная цель.** Если $P^* \leq 2/7$, никакое конечное $x$ не даёт жизнеспособности: подкритическую самомодель нельзя вскормить до сознания.
5. **Самосогласованный гейт: точная складка [Т в модели].** Возврат $\kappa_{\text{eff}} = \kappa \cdot g_V(P)\,(1 - 1/(7P))$ рождает живую стационарную ветвь через седло-узел, и складка **точна**: при $s = 7P - 2$ и $c = 7P^* - 1$ самосогласованность вынуждает $\Lambda^2 = (s+1)/c$, а условие касания сводится к квинтике
$$
(2\Lambda - 1)\,(c^2\Lambda^4 - 1) \;=\; 4c\,\Lambda^2\,(1 - \Lambda),
$$
чей единственный корень $\Lambda^* \in (1/\sqrt{c};\, 1)$ даёт складку в замкнутой форме: $u^* = \dfrac{c\Lambda^{*2} + 1}{c\Lambda^*(1-\Lambda^*)(c\Lambda^{*2} - 1)}$, $P_{\text{fold}} = \dfrac{c\Lambda^{*2} + 1}{7}$. Классификация по целям полна и точна: при $c < c^\dagger = 392/121 = 2^3\cdot 7^2/11^2 \approx 3{,}240$ зажим $g_V$ на складке неактивен и правит корень квинтики; при $c \geq c^\dagger$ зажим достигается раньше — на зажатой ветви условие касания вырождается в квадратику $c\Lambda^2 + 2\Lambda - 1 = 0$, чья критическая точка выпадает из той ветви, так что складка сидит ровно на изломе $\Lambda^* = \sqrt{2/c}$: **живая ветвь рождается ровно на потолке рефлексивности** $P_{\text{fold}} = 3/7$ ($R = 1/3$), с элементарной формой $u^* = 3/(\sqrt{2c} - 2)$. На стыковой цели $P^{*\dagger} = 513/847 \approx 0{,}606$ обе формулы дают $u^* = 11/2$ точно. На потолке Goldilocks $c = 2$: $8\lambda^5 - 4\lambda^4 + 8\lambda^3 - 8\lambda^2 - 2\lambda + 1 = 0$, $\Lambda^* = 0{,}858013$, $u^* = 21{,}4811$, $P_{\text{fold}} = 0{,}3532$ — квинтика и прямой численный поиск складки согласуются до $10^{-11}$ (спот-чеки: $c = 1{,}5$: $u^* = 75{,}371$ до $10^{-10}$; $c = 3$: $6{,}61011$ до $10^{-13}$; режим ребра $c = 4$: $3{,}62132$, $c = 6$: $2{,}04904$, со слиянием стационарных корней у $P = 3/7$). **Галуа-статус [Т]:** квинтика при $c = 2$ неприводима над $\mathbb{Q}$ с группой Галуа $S_5$ (порядок $120$), следовательно **в радикалах неразрешима**: константа складки — алгебраическое число пятой степени, не допускающее радикального выражения, — в контрасте с полом без гейта $x_{\min} = 1 + \sqrt{2}$, который радикален. «Замкнутая форма» для самосогласованной границы мёртвой зоны доказуемо обрывается на самой квинтике. Эта складка — микроскопическая граница Фазы III, реализующая в точности седло-узловую нормальную форму [бифуркационного анализа](/docs/applied/coherence-cybernetics/bifurcation); эвристика $r_c = P_{\text{crit}}/(7P)$ промахивается мимо неё на два порядка ($0{,}116$ против $21{,}5$) и должна читаться как маркер топологии, а не численная граница.
:::

**Доказательство (1)–(4).** Дефазировка не трогает диагональ, а замещение ведёт её к диагонали цели, так что $\mathrm{diag}(\Gamma_\infty) = 1/7$ точно. Каждая внедиагональ подчиняется $\dot{\gamma}_{ij} = -\Gamma_2\gamma_{ij} + \kappa_{\text{eff}}(\gamma^*_{ij} - \gamma_{ij})$ — линейной релаксации, чья неподвижная точка удерживает долю $\lambda = x/(1+x)$ когерентности цели. Тогда $P_\infty = 1/7 + \lambda^2 C^*$ немедленно; пол и потолок переводят $P_\infty \gtrless$ пороги через строго монотонную $\lambda(x)$, давая указанные концы; на равнонаселённой страте $\Phi = 7P - 1$, так что $\Phi$-пол и $P$-пол совпадают. При $P^* \leq 2/7$ даже совершенное удержание $\lambda = 1$ даёт $P_\infty = P^* \leq 2/7$. $\blacksquare$

**Машинная верификация.** Концы окна точны до $2\cdot10^{-3}$ на свипе из $4\cdot10^5$ точек ($x_{\min} = 2{,}4144$ против $1+\sqrt{2} = 2{,}414214$; двустороннее окно $(1{,}2677;\ 3{,}7749]$ при $P^* = 0{,}60$); складка гейта подтверждена двумя сливающимися стационарными корнями.

Под [гранд-каноническим словарём (T-258)](/docs/applied/coherence-cybernetics/sensorimotor#теорема-термодинамическая-трихотомия) $r$ — химический потенциал фазовой диаграммы сознания, а $x_{\min}$ — **порог конденсации**: структура устаивает против тепла лишь когда приток вещества превышает точку испарения, — а для сверхчистых идеалов лишь пока он остаётся *ниже* точки, где питание вымораживает рефлексивность.
...
Начиная с [T-258](/docs/applied/coherence-cybernetics/sensorimotor#теорема-термодинамическая-трихотомия), строка «$\mu_B \leftrightarrow r$» — больше, чем визуальная параллель: регенеративный канал **и есть** канал химической работы гранд-канонической трихотомии — единственный, ввозящий негэнтропию, — так что оси $(t, r)$ этой диаграммы суть буквально гранд-канонические $(T, \mu)$. То же отождествление независимо возникает в «Самообучающейся Вселенной» Ванчурина (2026), где химический потенциал — сопряжённая по Лежандру числа быстрых степеней свободы, третий и последний интенсивный параметр, который может нести обучающаяся система с ограниченными ресурсами.
