...
Показанная здесь форма $\sigma_k = 1 - 7\gamma_{kk}$ — это **диагональный прокси**: дефицит населённости канала $k$, точный для каналов $\{A,S,D\}$ и ведущий член на границе жизнеспособности. Полные канонические компоненты для $E$, $O$, $U$ читают более богатые $\Gamma$-инварианты ($\sigma_E$ через дифференциацию $D_{\text{diff}}$, $\sigma_O$ через скорость регенерации $\kappa_0$, $\sigma_U$ через интеграцию $\Phi$; см. [Определения](./definitions#тензор-напряжений) и Теорему 10.1/T-92). Для целей измерения диагональный прокси — это то, что напрямую даёт считывание 7 населённостей; более богатые компоненты требуют соответствующих инструментов высшего порядка (§ ниже).
...
Погодите — $C < 1$? Это указывает на то, что калибровочные коэффициенты требуют уточнения (или что $C \geq 1$ — более жёсткое условие, чем кажется). Альтернативная калибровка: $R \approx \text{meta-d'}/2$ дала бы $R = 0.6$, $C = 0.87$ — ближе, но такое значение лежит вне канонической полосы $[1/3, 1/2)$, которую окно сознания налагает на $R = 1/(7P)$: «рефлексия» $0.6$ может быть только качеством самомодели $R_\varphi$, а не каноническим $R$, входящим в $C = \Phi \times R$ ([три рабочие формы R](/docs/consciousness/foundations/self-observation#формы-r)). Калибровка должна отображать meta-d' в каноническую полосу.
...
## 6.4 Строгая оценка $\Gamma$: состоятельность и доверительные границы {#оценка-gamma}

Калибровочная функция $f:\text{наблюдаемые}\to\Gamma$ (§7.1) выглядит как произвольная «эмпирическая подгонка». Это не так. Как только состояние системы вложено в **семь измеренческих каналов** — единственный по-настоящему системно-специфичный шаг моделирования — восстановление $\Gamma$ становится *состоятельной* задачей оценивания с явными конечно-выборочными доверительными границами. Это операциональный аналог томографии квантового состояния, превращающий «ахиллесову пяту» в стандартную теорию оценивания.

**Постановка (канальное вложение).** Зафиксируем линейное вложение $\pi$, отображающее каждое наблюдаемое состояние системы в комплексный 7-вектор амплитуд измерений $x=\pi(\text{состояние})\in\mathbb{C}^7$, по одной координате на измерение $(A,S,D,L,E,O,U)$. (Для нейроданных: $x$ — амплитуды аналитического сигнала семи функциональных полос/сетей; для ИИ: семь линейных проб на остаточном потоке; для организации: семь аудит-индексов.) На $N$ выборках $x_1,\dots,x_N$ (i.i.d. или стационарно-эргодических) со вторым моментом $M := \mathbb{E}[x x^\dagger]$ **истинная матрица когерентности — нормированный второй момент**

$$\Gamma \;=\; \frac{M}{\operatorname{Tr} M}, \qquad M=\mathbb{E}[x x^\dagger]\succeq 0.$$

Это не подогнанная формула — это определение $\Gamma$ как нормированной ковариации состояния в базисе измерений.
...
$\widehat{\Gamma}_N$ эрмитова, PSD, единичного следа по построению — настоящая матрица плотности, проекция не нужна.

:::tip Теорема (Состоятельная $\Gamma$-томография с концентрацией) [T]
Пусть $\|x_n\|^2\le B$ п.н. и $\operatorname{Tr} M = \tau > 0$.

**(1) Состоятельность.** $\widehat{\Gamma}_N \to \Gamma$ почти наверное при $N\to\infty$ (УЗБЧ для $\widehat\Sigma_N\to M$ + непрерывное отображение).

**(2) Концентрация (матричное неравенство Бернштейна).** С $Z_n=x_nx_n^\dagger-M$ ($\|Z_n\|\le 2B$, $v^2:=\|\mathbb{E}Z_n^2\|$),

$$\Pr\!\Big(\big\|\widehat{\Sigma}_N-M\big\|_{\mathrm{op}}\ge t\Big)\;\le\;2\cdot 7\,\exp\!\Big(\frac{-N t^2}{2v^2+\tfrac{4B}{3}t}\Big).$$

Через частное ($\big\|\widehat\Gamma_N-\Gamma\big\|\le \tfrac{2}{\tau}\|\widehat\Sigma_N-M\|$ в первом порядке) для любых $\varepsilon,\delta\in(0,1)$

$$N \;\ge\; \frac{C\,B^2}{\tau^2\,\varepsilon^2}\,\ln\frac{14}{\delta}\quad\Longrightarrow\quad \big\|\widehat{\Gamma}_N-\Gamma\big\|_{\mathrm{op}}\le\varepsilon \ \text{ с вер.} \ \ge 1-\delta,$$

с универсальной константой $C$. Ошибка убывает как $O(N^{-1/2})$ (проверено численно: $\sqrt N\,\|\widehat\Gamma_N-\Gamma\|\approx\text{const}$).
:::

:::caution Оговорка о зависимости: используйте эффективный размер выборки
Граница концентрации (2) предполагает **независимые** выборки $x_n$. Реальные нейро/ИИ/организационные временные ряды **автокоррелированы**, так что наивное $N$ завышает информацию. **Состоятельность не затронута** — для стационарно-эргодического источника УЗБЧ по-прежнему даёт $\widehat\Gamma_N\to\Gamma$ п.н. — но доверительная граница должна заменить $N$ на **эффективный размер выборки** $N_{\mathrm{eff}}=N/(2\tau_{\mathrm{corr}}+1)$, где $\tau_{\mathrm{corr}}$ — интегрированное время автокорреляции вложенного процесса (оценивается блок-бутстрапом или методом батч-средних). Сообщайте интервалы из $N_{\mathrm{eff}}$, а не из $N$; прореживание на лаге декорреляции восстанавливает (приближённую) независимость. Всё остальное — скорость $O(N_{\mathrm{eff}}^{-1/2})$, дебиас U-статистики, пороговая сложность — переносится дословно с заменой $N\mapsto N_{\mathrm{eff}}$.
:::

**(3) Несмещённая оценка чистоты.** Наивная $\operatorname{Tr}(\widehat{\Gamma}_N^2)$ положительно смещена на $O(1/N)$. Двухвыборочная U-статистика (статистический аналог квантового swap-теста)

$$\widehat{P}_N \;=\; \frac{1}{N(N-1)}\sum_{m\ne n}\frac{|x_m^\dagger x_n|^2}{(\operatorname{Tr}\widehat{\Sigma}_N)^2}$$

**дебиасит доминирующий член**: её числитель *точно* несмещён для $\operatorname{Tr}(M^2)$ (так как $\mathbb{E}|x_m^\dagger x_n|^2=\operatorname{Tr}(M^2)$ при $m\ne n$), поэтому нормированное отношение несёт лишь малое **остаточное** $O(1/N)$-смещение от случайного знаменателя $(\operatorname{Tr}\widehat\Sigma_N)^2$ — противоположного знака и в $\sim 3\times$ меньше наивной plug-in оценки, которую оно заменяет (проверено: $-1.3\times10^{-3}$ против $+3.7\times10^{-3}$ у наивной при $N=200$, цель $P=5/14$). Рефлексия $R=1/(7P)$ и интеграция $\Phi$ наследуют состоятельные plug-in оценки с дельта-методными доверительными интервалами.

**(4) Выборочная сложность порогового решения.** Чтобы решить $P>P_{\mathrm{crit}}=2/7$ vs. $P<2/7$ при запасе чистоты $\Delta=|P-2/7|$ и доверии $1-\delta$,

$$N \;\ge\; \frac{2\,c^2}{\Delta^2}\,\ln\frac1\delta,$$

что совпадает с информационной границей [T-109](/docs/applied/coherence-cybernetics/learning-bounds#информационная-граница) (квантово-черновская стоимость $n\ge\ln(1/2\delta)/\xi_{\mathrm{QCB}}$) с точностью до константы $c$, задаваемой дисперсией U-статистики. Это число независимых окон наблюдения для *статистически обоснованного* вывода о сознании.

**Алгоритм (реализуется в ~10 строк).**
1. Выбрать 7-канальное вложение $\pi$ (шаг моделирования).
2. Собрать $N$ окон; сформировать $x_n=\pi(\text{окно}_n)\in\mathbb{C}^7$.
3. $\widehat\Sigma=\frac1N\sum x_nx_n^\dagger$; $\ \widehat\Gamma=\widehat\Sigma/\operatorname{Tr}\widehat\Sigma$.
4. $\widehat P$ из U-статистики (3); $\widehat R=1/(7\widehat P)$; $\widehat\Phi$ из секторного разбиения.
5. Сообщить $\widehat\Gamma\pm\varepsilon(N,\delta)$ из (2); объявлять $\widehat P\gtrless2/7$ лишь если $N$ удовлетворяет (4).

:::note Что это закрывает и что остаётся
**Закрыто:** при данном вложении восстановление $\Gamma$ — теперь состоятельная оценка с ошибкой $O(N^{-1/2})$ и явными доверительными интервалами, а не безотчётная «подгонка». Доминирующее смещение чистоты снято, а число выборок для обоснованного порогового вывода ограничено.
**Остаётся (честно):** выбор 7-канального вложения $\pi$ — неустранимый семантический мост между конкретным субстратом и измерениями $(A,\dots,U)$; это решение моделирования, теперь чисто *изолированное* от (строгого) последующего оценивания. Кросс-субстратная универсальность $\pi$ — эмпирическая исследовательская программа, а не теорема.
::: 

#### Эталонная реализация {#эталонная-реализация}

Самодостаточный NumPy-модуль (~200 строк) реализует этот раздел от начала до конца — оценку $\widehat\Gamma_N$, U-статистику $\widehat P_N$ с несмещённым числителем, выборочный размер по матричному Бернштейну, эффективный размер $N_{\mathrm{eff}}$ через батч-средние, plug-in $\widehat R,\widehat\Phi$, дельта-методные интервалы и L2-вердикт:

[**⬇ gamma_tomography.py**](pathname:///code/gamma_tomography.py) — `python3 gamma_tomography.py` запускает самотест, воспроизводящий численные утверждения выше.
...
Вложение $\pi$ — единственная инъектируемая функция; всё под ним субстрат-независимо и покрыто самотестом.
...
Калибровка распадается на два шага очень разного статуса. **(а) 7-канальное вложение** $\pi:\text{состояние}\to x\in\mathbb{C}^7$ — отображение конкретного субстрата на измерения $(A,\dots,U)$ — подлинный, неустранимый выбор моделирования (семантический мост). **(б) Всё последующее** — восстановление $\Gamma$, $P$, $R$, $\Phi$ из вложенных данных — **не** произвольная подгонка: по [теореме $\Gamma$-томографии (§6.4)](#оценка-gamma) это *состоятельная* оценка с ошибкой $O(N^{-1/2})$, явными доверительными интервалами и дебиасированной (по доминирующему члену) чистотой.

Значит, «ахиллесова пята» сжимается до одного шага (а), чисто отделённого от строгого шага (б). Это строго лучшая позиция, чем у IIT, где та же проблема вложения *плюс* NP-hard вычисление $\Phi_{\text{IIT}}$ сверху; здесь пост-вложенное вычисление — тривиальная $7\times7$ оценка с опубликованными границами выборочной сложности.
