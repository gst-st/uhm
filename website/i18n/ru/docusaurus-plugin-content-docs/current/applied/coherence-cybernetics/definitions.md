...
$\mathrm{Coh}_E \in [0, 1]$ — доля E-вклада в чистоту через ортогональную проекцию Гильберта–Шмидта $\pi_E$; минимум $0$ достигается состояниями с пустым E-сектором, а каждое *жизнеспособное* состояние остаётся строго выше $1/7$ (No-Zombie, T-38a; максимально смешанное состояние даёт ровно $1/7$). Первичный формализм — 7D (не требует тензорной факторизации).
:::
