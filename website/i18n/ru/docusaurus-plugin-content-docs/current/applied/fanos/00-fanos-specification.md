...
**Порог системной корреляции `r* = 1/√6 ≈ 0.408` [Т]** (числа — для базовой 7-ячейки `q=2`; при общем `N` пороги масштабируются: `r* = 1/√(N−1)`, `P_crit = 2/N`)**.** На равнокоррелированной страте — все внедиагональные корреляции равны средней `r` — меры сворачиваются в замкнутую форму (проверено `fanos_verify.py` V15):
