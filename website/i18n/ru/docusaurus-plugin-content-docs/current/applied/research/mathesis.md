...
Программный код (Rust + TypeScript) — конечная аппроксимация бесконечного объекта $\mathfrak{M}$, подобно тому как численное решение дифференциального уравнения аппроксимирует непрерывную динамику. Аппроксимация может улучшаться; $\mathfrak{M}$ остаётся неизменным.
...
Текущая вычислительная реализация (гиперграф, SQLite, Rust) аппроксимирует $\mathfrak{M}$ на 1-категориальном уровне. Три направления расширения устраняют фундаментальные ограничения.
...
**Связь с УГМ.** Это не аналогия — прямой перенос. $\Gamma \in \mathcal{D}(\mathbb{C}^7)$ описывает сознательное состояние; $\rho_{\text{ep}} \in \mathcal{D}(\mathbb{C}^k)$ описывает **эпистемическое состояние**. Формулы одинаковы, потому что математическая структура одна: ∞-топос для физики ($\mathfrak{T}$) и для эпистемологии ($\mathfrak{M}$).
...
- **Ядро Матезиса** — три движка: Движок фибрации хранит и обходит гиперграф; Эпистемический движок проверяет и пропагирует статусы; Слой агента Claude выполняет семантические операции. Язык: Rust (ядро) + TypeScript (МП-обёртка).
...
- Rust-крейт `mathesis-core`: гиперграф, фибрация, когерентность, пропагация
- Rust-крейт `mathesis-index`: сканирование markdown → гиперграф
...
Матезис реализуется целиком на **Verum** — языке программирования, спроектированном как язык предельной математической полноты и производительности. Verum — единственный язык, совмещающий зависимые типы, SMT-верификацию, системное программирование, GPU compute и ∞-категорную математику в одном стеке.
...
**Зависимые типы** (crates/verum_types, помечены v2.0+):
