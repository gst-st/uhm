...
Пусть $g_{\mathrm{B}}$ — метрика Бюреса на $\mathcal{D}_+(\mathbb{C}^N)$: она связана с SLD-метрикой квантовой информации Фишера, но **не равна** ей — $\mathcal F_{\mathrm{SLD}}=4g_{\mathrm{B}}$ (Braunstein–Caves 1994, как и используется в [T-187 Char-IV](/docs/proofs/categorical/cohesive-closure)), так что на коммутирующих возмущениях $g_{\mathrm{B}}=\tfrac14\,$Фишера–Рао. Тогда:
...
:::warning Здесь нормировка несёт нагрузку
Скалярная кривизна **не** масштабно-инвариантна: при $g\mapsto c\,g$ она преобразуется как $R_{\mathrm{scal}}\mapsto R_{\mathrm{scal}}/c$. Поэтому и оценка $N(N-1)/8$, и величина $21/4$ даны в буресовой нормировке Хюбнера и не должны приводиться рядом с метрикой в нормировке SLD-QFI, где они отличались бы в четыре раза. Свободны от нормировки лишь два *качественных* факта, которые используются дальше: кривизна ограничена снизу и расходится при коллапсе ранга, — и именно на них опирается аргумент о хирургии.
:::
