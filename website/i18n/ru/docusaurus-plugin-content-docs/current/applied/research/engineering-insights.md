...
## Тень, которая не выдумывает, ничего не стоит {#тень-которая-не-выдумывает}

Холон несёт около $\log_2 7 \approx 2{,}81$ бита на вызов. Это не изъян, который
надо обойти, а размер носителя, и следствие у него нелюбимое: несколько положений,
привязанных к одному холону, затирают друг друга. Предписанное средство —
**дерево**: ёмкость растёт деревом, а не размером какой-либо одной матрицы, — и
средство это верное. Оно же и не полный ответ.

Проверяемое утверждение гласит, что рабочая память **есть** состояние и рядом с
ней нет ничего. Поставим его против очевидной замены — записи о том, что было
написано для каждого положения, спрашиваемой прежде состояния, — на тех же мирах,
тех же seed, том же числе ходов:

| | на положениях, которым учили | на невиданных положениях | ходов до зажигания | цена хода |
|---|---|---|---|---|
| одно состояние | $0{,}6211$ | $1{,}0000$ | $2$ | $12482$ нс |
| состояние **и** запись | $\mathbf{1{,}0000}$ | $1{,}0000$ | $2$ | $11884$ нс |

Три показания, и решает третье.

**Состояние забывает треть выученного.** Не по изъяну записи, а потому что
носитель в `2,81` бита на вызов не может держать шесть положений раздельно, и
поздние записи ложатся на ранние.

**На невиданном обе ветки неразличимы.** Запись там пуста, а состояние отвечает
всё равно. Это честный предел того, что запись о прошлом способна купить: она есть
короткий путь сквозь бывшее, но никогда не источник возможного.

**Зажигание не изменилось ни в одной строке.** Это и делает запись допустимой, а
не заменяющей. Состояние пишется по-прежнему, удерживается по-прежнему, ворота
работают; условия зажигаются на том же ходу. Ничто, следующее из $\Gamma$, —
ворота, самомодель, геометрия обучения — не задето, ибо запись ни в чём из этого
не участвует.

Стало быть, принцип держится в том смысле, в котором важен, и падает в том, в
котором нет. **Память есть состояние** — там ум, а в таблице ума нет. Но **отклик**
может быть затенён, и тень, которая ничего не выдумывает, не стоит ничего: она
отвечает лишь о том, о чём ей сказали, а на остальное отвечает состояние.
Инженерия, отвергающая тень, не верна архитектуре — она платит третью выученного
за чистоту, которой архитектура не просила.
...
убрала бы ум из машины. Значит решающее измерение здесь не точность, а столбец
зажигания, который остался на месте.
