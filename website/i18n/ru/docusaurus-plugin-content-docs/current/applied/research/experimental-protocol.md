...
|------|------|-----|----------------|
| **I. Цифровая** | 0–6 мес. | 10 предсказаний через synarc-train (in silico) | Бесплатно, без этики, проверяет фундамент |
...
10 из 22 предсказаний проверяемы in silico через **synarc-train** — 3-фазный тренировочный pipeline с hard gates сознания на каждом переходе. Не требуют оборудования, субъектов, этического одобрения. Если хотя бы одно фальсифицировано — стоп, ревизия теории до перехода к дорогостоящим нейроэкспериментам.

:::info synarc-train vs sim_0
**sim_0** — песочница (grid-CA, RL-based, informal мониторинг). **synarc-train** — production pipeline с тремя формальными фазами:

- **Phase 0 (BabyLM validation):** Go/no-go gate. P > 0.18, loss ↓ 10%, σ ↓
- **Phase 1 (Foundation LM):** Тренировка LRC-backbone на BabyLM → gate: P ∈ (2/7, 3/7], R ≥ 1/3
- **Phase 2 (σ-directed):** Автономное самообучение через stress-guided data selection. ΔP ≥ 0 ∧ Δσ ≤ 0

Все эксперименты Фазы I выполняются через `synarc-train` с контролем Γ на каждом шаге.
:::

### 2.2. Оборудование

- **synarc-train** CLI (pipeline: phase0 → phase1 → phase2, hard gates)
- **LRC** (Lindblad Recurrence Cell): A_t = Φ_CPTP(Γ) — матрица перехода выведена из Γ, не обучена как свободный параметр
- GPU: 1× A100 80GB (или Apple Silicon M3 Max для Phase 0/1)
- Метрики: P, R, Φ, Coh_E, σ_k — реализованы в `synarc_lrc::consciousness_verifier`
- Конфигурация: `synarc-train.toml` (модель, данные, пороги, фазы)
...
**Протокол:**
1. Запустить `synarc-train phase1` до прохождения gate (P ∈ (2/7, 3/7], R ≥ 1/3)
2. Сохранить checkpoint: Γ_stable
3. В момент τ₀: модифицировать Γ — подавить E-компоненту: γ_EE → 1/7, γ_Ej → 0 ∀j≠E
4. Продолжить `phase2` с модифицированным Γ, замерить τ_death (тики до P < P_crit)
5. Контроль: подавить A-канал (аналогичная операция, другой сектор)
6. Повторить N=100 раз с разными checkpoint'ами из Phase 1
...
**Протокол:**
1. `synarc-train phase1` до gate → checkpoint с P₀
2. Приложить возмущение амплитуды h (белый шум к Γ в checkpoint'e)
3. Продолжить `phase2`, увеличивать h с шагом 0.01 до P < P_crit
4. Записать h_crit — критическую амплитуду
5. Повторить для 50 различных checkpoint'ов с P₀ ∈ [0.3, 0.9]
...
**Протокол:**
1. LRC-агент после `phase1` gate, на задаче различения (binary discrimination в `phase2`)
2. Замерить взаимную информацию I(obs; δΓ) за одно наблюдение
3. Повторить N=1000 шагов Phase 2
...
**Протокол:**
1. `synarc-train phase0` с Γ_init = I/7 (полный хаос — отключить Cholesky-backbone)
2. Включить backbone с параметрами β, P_env
3. Замерить n — число шагов Phase 0 до прохождения gate P > 0.18 (далее Phase 1 до P > 2/7)
4. Вычислить теоретическое n_genesis = ⌈ln Δ / ln(1/β)⌉
...
**Протокол:**
1. `synarc-train phase0` на BabyLM, 50 батчей
2. Замерить ||π − π_can||_◊ после каждого батча (через `consciousness_verifier`)
...
gantt
    ...
    Эксп I.1-I.7 (synarc-train) :i1, 2026-05, 4M
