...
УГМ — одна из наиболее формально проработанных теорий сознания: ~210 теорем, 22 числовых предсказания, категорный фундамент. Но **ни одно предсказание не проверено экспериментально**. Теория без эмпирики — философия, какой бы строгой ни была математика.
...
| Калибруемость | [H] | Требует эмпирической подгонки $\beta$ под соответствие PCI* ↔ P_crit |

**Статус:** Конструкция **формально корректно определена** [Т] (шаги 1–4 — явные CPTP-операции). **Калибровка** (выбор $\beta$ и популяционных норм $\mu_k, \sigma_k$) эмпирична [H] — требует завершения эксперимента Фазы II (§3.3). После калибровки π_bio становится валидированным измерительным инструментом [Т].
