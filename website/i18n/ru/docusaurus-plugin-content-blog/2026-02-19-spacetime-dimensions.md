...
### Почему три пространственных? {#почему-три-пространственных}

Есть соблазнительный короткий ответ, и он неверен. Соблазнительный звучит так: у фундаментального представления $SU(3)$ размерность 3 — не 2, не 4, — вот вам и пространство.

Не следует. Размерность представления считает, сколько внутренних компонент едет *над* точкой; пространственная размерность — по скольким направлениям точка может *двигаться*. Это разные вопросы, и подмену легко поймать: положите перед собой решётку $\mathbb{Z}^d$ и повесьте над каждым узлом $n$-компонентное внутреннее пространство. Посчитайте собственные значения. Показатель роста всякий раз выходит равным $d$ и к $n$ совершенно безразличен — внутренняя размерность лишь умножает, *сколько* собственных значений набирается на каждом уровне, но никогда — как быстро растёт их число. Значит $\dim(\mathbf{3}) = 3$ не может быть причиной трёхмерности пространства.

Настоящая причина острее, и она про *одновременность*.

Чтобы приписать состоянию точку $\mathbb{R}^k$, нужно считать с него $k$ чисел **сразу**. В квантовой механике это значит $k$ коммутирующих наблюдаемых — иначе нет совместного отсчёта, нет координат, нет точки. Поэтому вопрос не «насколько велика алгебра пространственных наблюдаемых?», а «сколько её элементов измеримо вместе?». Это число есть **ранг** алгебры.

Проделайте это на самих октонионах. Зафиксируйте $O$-направление; его стабилизатор внутри группы автоморфизмов есть $\mathfrak{su}(3)$, восьмимерная. Спросите, что коммутирует со всей ею на оставшихся шести направлениях, — и выпадает комплексная структура $J$ с $J^2 = -1$, не навязанная, а *выведенная*. Значит пространственная алгебра наблюдаемых есть $\mathfrak{su}(3) \oplus \mathfrak{u}(1)_J = \mathfrak{u}(3)$ размерности девять. Теперь возьмите типичный элемент и спросите, какая часть алгебры с ним коммутирует. Ответ — **три**, каждый раз.

$$
d_{\text{пространство}} \;=\; \operatorname{rank}\,\mathfrak{u}(3) \;=\; 3
$$

Посмотрите, что это отсекает. $\dim\mathfrak{u}(3) = 9$, $\dim\mathfrak{su}(3) = 8$, $\dim G_2 = 14$ — ни одно из них не равно 3. Только ранг. **Пространство трёхмерно потому, что три — наибольшее число вопросов, которые можно задать голону одновременно.**

И ещё один поворот винта, на котором результат перестаёт быть бухгалтерией. Эти три направления независимы лишь потому, что $\mathbf{3}$-сектор сидит внутри *семимерного* целого, оставляющего его суммарный вес свободно дрейфовать. Измерьте те же три наблюдаемые внутри одного $\mathbf{3}$-блока, запечатанного, — и одна из них перестаёт флуктуировать: выйдет два измерения, а не три. Чтобы третья координата вообще была живой, амплитуда должна иметь возможность перетекать между пространственным сектором и часами.

Значит часы — не четвёртая вещь, стоящая рядом с тремя пространственными. Они резервуар, без которого пространство было бы плоскостью. $(1,3)$ — это не $1+3$, а сцепленная пара, и, убрав $1$, вы не останетесь с трёхмерным миром.

Три пространственных измерения поэтому не эстетический выбор и не антропная тавтология. Они — ранг, а октонионы задают, какой именно, поскольку $\mathbb{O}$ есть единственная неассоциативная нормированная алгебра с делением ([теорема Гурвица](/docs/proofs/minimality/theorem-minimality-7), 1898 [Т]). Уберите измерение — и вы падаете обратно к кватернионам, теряя неассоциативность, а с ней всю структуру оператора Gap. Добавьте — идти некуда: после $\mathbb{O}$ нормированные алгебры с делением кончаются.

Полный вывод с машинными проверками: [Обучающий алгоритм голона](/docs/applied/research/vanchurin-bridge) — соседний результат на том же секторе; сам ранговый расчёт и демонстрация схлопывания до двух — в [Эмерджентном многообразии](/docs/proofs/physics/emergent-manifold#шаг-2c-ранг).
...
| Расщепление $(1,3)$ — одни часы, три пространственных | [Т] | Ранги: $\operatorname{rank}\mathfrak u(1)_O = 1$, $\operatorname{rank}\mathfrak u(3) = 3$ |
| Лоренцев *знак* $(+,-,-,-)$, а не евклидов | [С] | Крейнова тройка T-53 с ним согласована, но его не выделяет; знак опирается на ограниченность $H_S$ снизу (положительность отражения) |
| Время из O через Пейдж–Вуттерс | [Т] | Эмерджентное, не постулированное |
| Стрела времени — коллапс страт | [Т] | $\dim(X_\tau) \geq \dim(X_{\tau+1})$ |
| Пространство из $\{A,S,D\}$ — некомпактный сектор | [Т] | $\operatorname{rank}\mathfrak u(3) = 3$, нет Хиггса в $SU(3)_C$ |
| Компактификация $\{L,E,U\}$ через Хиггс | [Т] | $v_{\text{EW}} \sim 246$ ГэВ, нарушение $SU(2)_L \times U(1)_Y$ |
| Фоновая независимость ($M^4$ выведено) | [С] | Цепочка Гельфанда–Наймарка–Конна (T-120). Шаг Гельфанда и счёт размерности — [Т]; сама *реконструкция* многообразия оставляет открытыми две из семи аксиом Конна (первопорядковое условие, двойственность Пуанкаре) |
...
| $M^4$ фоново-независимо? | Выведено из категорной структуры (T-120) | [С] — размерность [Т], реконструкция открыта |
...
**5. Пространство-время — не сцена, а следствие.** Гладкое многообразие $M^4 = \mathbb{R} \times \Sigma^3$ **выведено** из категорной структуры [С] (T-120 — размерность [Т], реконструкция многообразия открыта): макроскопическая алгебра наблюдаемых в $\{A,S,D\}$-секторе коммутативна в термодинамическом пределе (квантовая центральная предельная теорема, T-117 [Т]); по дуальности Гельфанда–Наймарка коммутативная C*-алгебра изоморфна $C(\Sigma^3)$ для единственного гладкого 3-многообразия (T-119 [Т]); произведение с временно́й компонентой даёт $M^4$ (T-120 [Т]). Пространство — не данность, а эмерджентная структура. Эйнштейн деформировал сцену; УГМ выводит её из-за кулис.
