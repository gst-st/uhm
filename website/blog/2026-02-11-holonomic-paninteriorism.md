...
Interiority (L0) — the presence of an "interior" — belongs to **every** system described by a coherence matrix $\Gamma$, including the maximally mixed state $I/N$. This is a universal property — a consequence of Axiom Ω⁷. But consciousness (L2) requires three threshold conditions simultaneously: reflection $R \geq 1/3$, integration $\Phi \geq 1$, differentiation $D_{\text{diff}} \geq 2$. An electron has interiority ($\mathrm{L0} = \mathrm{true}$), but not consciousness ($\Phi \ll 1$): its $R$ is not merely high, it is *maximal* — for $I/7$ the purity is exactly $P = 1/7$, so $R = 1/(7P) = 1$, and since $P \geq 1/7$ for every state, $1$ is the largest value $R$ can ever take. The state with the most formal reflection in the entire theory is the state with nothing whatsoever to reflect. The electron has an "interior" — but there is no one who knows about it.
