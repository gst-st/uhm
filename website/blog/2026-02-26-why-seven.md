...
### A Third Lock, and This One You Can Check Yourself {#третий-замок}

Tracks A and B are proofs you read. There is a third lock on the number seven that you can *run*, and it closes the question differently: not "why is seven allowed?" but "how many ways are there to be seven?"

**One.** Exactly one.

The coordination structure UHM needs at $N=7$ is a design in which every pair of dimensions meets in exactly one shared line — the Fano plane $\mathrm{PG}(2,2)$, formally a BIBD$(7,3,1)$. It is natural to ask whether that design is a choice among many. Enumerate them exhaustively: take all $\binom{7}{3} = 35$ triples, keep every family of seven that covers each of the 21 pairs exactly once. The search returns **30** labelled designs. Now count the symmetries of any one of them: $\lvert\mathrm{Aut}\rvert = 168$, which is $\lvert\mathrm{GL}(3,2)\rvert = (8-1)(8-2)(8-4)$. And $7!/168 = 30$ — precisely the number found. One orbit. **Every** BIBD$(7,3,1)$ is the same design relabelled; there is nothing to choose.

The same holds one level down, in the algebra. Build the octonions from scratch by Cayley–Dickson doubling and ask what their derivation algebra is — the infinitesimal symmetries, the thing whose exponential is $G_2$. Set up the Leibniz condition as a linear system on $7\times7$ matrices and take the null space. Out comes dimension **14**, exactly $\dim G_2$, with alternativity and norm-multiplicativity holding to $10^{-14}$ and associativity failing outright. Nothing was assumed about $G_2$; it was computed.

So the seven is locked three times over: remove a dimension and function fails (Track A), add one and algebra fails (Track B), and *at* seven the coordinating design and its symmetry group are not choices but the unique solutions.
...
| 3 spatial dimensions | $\operatorname{rank}\mathfrak u(3) = 3$ for $SU(3) \subset G_2$ | [Post 5](/blog/spacetime-dimensions) |
