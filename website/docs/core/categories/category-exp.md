...
### Пространство контекстов $\mathcal{C}$: классические параметры

$$
\mathcal{C} = \{\text{состояния всех измерений кроме } E\}
$$
...
Контекст наделён **дискретной метрикой**: $d_\mathcal{C}(c_1, c_2) = 0$ если $c_1 = c_2$ и $d_\mathcal{C}(c_1, c_2) = \infty$ иначе. Это отражает тот факт, что классические параметры не имеют квантовой геометрии — «между» двумя контекстами нет промежуточных состояний.
...
**Компонент $h$: преобразование контекста.** Отображение $h: \mathcal{C} \to \mathcal{C}$ меняет «декорации сцены» — состояния измерений A, S, D, L, O, U. Поскольку контекст имеет дискретную метрику, $h$ — это «переключение» между дискретными конфигурациями, а не плавное изменение.
