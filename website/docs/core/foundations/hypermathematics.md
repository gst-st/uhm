...
:::note Where these verifiers now live

The four scripts named above — `genesis_seven_verify.py`, `gauge_uniqueness_verify.py`, `genesis_wave4_verify.py`, `genesis_wave6_verify.py` — were **consolidated** and no longer exist under those names. Their checks live in the two verifiers shipped with the math-foundations corpus:

* `static/verifiers/hypermath_verify.py` — **45/45 PASS** (2026-07-28), including the $\mathbb{Q}$-ouroboros and IVT-failure witnesses and the $\mathfrak{g}_2$ machinery ($\dim = 14$, the stripping ladder $8/4/3/0$, the mediator kill at $4.5\times10^{-17}$);
* `static/verifiers/foundations_verify.py` — **285/285 PASS** (2026-07-28).

Recorded here because a citation that names a file which cannot be found is not a verification a reader can repeat `[О]`.
:::
