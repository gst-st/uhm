...
:::tip Document status
This is a **meta-level** document: an **[О]**-instrument (its criteria are definitions) applied in **[И]/[Т]**-graded audits. It does not prove theorems; it audits *readings* — the organizing structures through which the corpus arranges its theorems — and issues verdicts with statuses. It binds future canonizations (§9). Everything it says about specific theorems carries the statuses those theorems already have in the [status registry](/docs/reference/status-registry).
:::
...
- **H1.2** The *stage* of the Universe — its purity $P$ on the ladder $P^{(n)}_{\mathrm{crit}}$ — was **neither derived nor measured** [Г]; **upgraded to [С] by [T-266](/docs/physics/gravity/cosmological-constant#теорема-стадия-вселенной)**. Two steps close the *value*: (Part A, [Т-structural]) any smooth functional of the exponentially relaxing vacuum $\Gamma$ inherits the envelope $e^{-\kappa\tau}$, so the drift law gives the fractional stage-distance $1-\mathcal{G}^\infty/\mathcal{G}_O = \tfrac{3H_0}{2\kappa}(1+w_0)$, which is $\ll 1$ for any microphysical $\kappa \gg H_0$ — the Universe sits at its *terminal* stage, and the DESI drift is the $\kappa/H_0$-amplified residual; (Part B, [С]) with $\kappa = \omega_0/7$ (T-59) and $\omega_0 \approx 2.4\times10^{-2}M_{\text{Pl}}$ (neutrino $M_{G_2}$ sector) the residual is $\sim 5\times10^{-60}$, and if the terminal state is the conscious-window attractor (T-124) then $P(\text{today}) = 3/7$ to $\sim 59$ figures. The stage is thus **derived** ($3/7$ [С]) **and measured** (the residual is read from $|1+w|$). The remaining [О] is the exponent's $\omega_0$-dependence and the magnitude of Λ itself (the ≳27-order deficit, a distinct problem). A distinct face — which structures *are* viable holons — is reduced to habitability science by [T-257 §4](/docs/applied/research/one-grammar#планетарная-система): the planetary boundary conditions that discharge the (Acc) clause of T-253 for a biological substrate.
...
|---|---|---|---|---|
| H1.1 | Universal $\rho^*$: internal terminality over sub-holons | I | **[С]** (T-248) | internal terminality (Universe terminal over its sub-holons, T-222 Pareto-optimum the sink), conditional on part→whole inclusions being resource morphisms + the Markov hypothesis; a computed cosmological T-222 analogue would give [Т] |
| H1.2 | The Universe's stage $P$ — value | I | **[С]** (T-266) | **value closed [С] by [T-266](/docs/physics/gravity/cosmological-constant#теорема-стадия-вселенной)**: Part A [Т-structural] — the vacuum relaxes exponentially, so $1-\mathcal{G}^\infty/\mathcal{G}_O = \tfrac{3H_0}{2\kappa}(1+w_0) \ll 1$ for any microphysical $\kappa\gg H_0$ ⟹ the Universe is at its *terminal* stage, the DESI drift being the $\kappa/H_0$-amplified residual; Part B [С] — $\kappa=\omega_0/7$ (T-59) + $\omega_0$ from the neutrino $M_{G_2}$ sector give residual $\sim5\times10^{-60}$, and $P^*=3/7$ (T-124) ⟹ $P(\text{today})=3/7$ to $\sim59$ figures. Derived **and** measured (residual read from $|1+w|$). Residual [О]: the exponent's $\omega_0$-dependence; the Λ-magnitude deficit (≳27 orders) is a *distinct* open problem, not the stage |
...
| H3.5 | Sphere-spectrum grounding of the genesis tower | III | **[Г]** (open, registered by [T-277–T-280](/docs/core/foundations/hypermathematics#поглощение)) | a functor from the mirror-tower/volume-law layer into stable homotopy making Adams' Hopf-invariant-one and Bott periodicity the *image* of the viability boundary — so that the deepest boundary-proofs become internal to the genesis rather than external certificates |
| H3.6 | Pre-metric base of the mirror tower | III | **substantially advanced** ([T-281](/docs/core/foundations/hypermathematics#единственность-калибровки): the metric layer — norm, composition — is *derived* from no-annihilations alone within the monomial class; viability linearizes via the rectangle rule) | the Cayley–Dickson tower starts at $\mathbb{R}$ (the fixed line of the first conjugation); a fully pre-metric start — the combinatorial skeleton (grading + cocycle) over $\Omega$ alone, with positivity/norm emerging rather than assumed — would push the genesis of [T-277](/docs/core/foundations/hypermathematics#терминальное-зеркало) one floor deeper |
