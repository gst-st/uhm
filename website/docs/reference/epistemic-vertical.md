...
- **H1.2** The *stage* of the Universe — its purity $P$ on the ladder $P^{(n)}_{\mathrm{crit}}$ — is **neither derived nor measured** [Г]. "The Universe is at a certain stage of viability" is structurally meaningful (the ladder exists [Т]) but numerically open. Closing it is a research object adjacent to the Λ-deficit programme. Since T-254, the hole has its first *observational estimators*: the dark-energy drift $|1+w(z)| = \tfrac{2}{3}|d\ln\mathcal{G}_O/d\ln a|$ measures the per-e-fold **rate** of the vacuum's stage motion, with $\dot G_N/G_N$ as its fixed-ratio companion (T-255) — the stage *value* itself remains open. A distinct face — which structures *are* viable holons — is reduced to habitability science by [T-257 §4](/docs/applied/research/one-grammar#планетарная-система): the planetary boundary conditions that discharge the (Acc) clause of T-253 for a biological substrate.
...
| H1.2 | The Universe's stage $P$ neither derived nor measured | I | [Г] | Λ-deficit programme output or an independent cosmological estimator; by T-244 the stage is the frame's degree of anisotropy, so the estimator target is sharpened (cosmic anisotropy ↔ $P$); by T-254/T-255 the stage's *drift rate* is already observable — $w(z)$ surveys and $\dot G_N$ bounds read $d\ln\mathcal{G}_O/d\ln a$ ([cosmological constant §13b](/docs/physics/gravity/cosmological-constant#desi-совместимость)) — the floor's sharpest open edge, and the residue of H1.3 |
