...
Значение $\lambda_3 \approx 74$ существенно превышает границу пертурбативной унитарности $\sim 4\pi \approx 12.6$. Это означает, что все петлевые вычисления, зависящие от $\lambda_3$ (массы лёгких поколений, $\varepsilon_{\text{eff}}$, $m_b/m_t$, CKM-углы), находятся в **непертурбативном режиме** и формально ненадёжны.
