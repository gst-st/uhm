...
**Moulting and the patrol.** When the world changes its law mid-life, the earned map begins to lie — and surprises turn out to be a poor judge of it. Continuous overwriting is itself a *local* moult: a surprise fires only on the first meeting with each spoiled edge, so the lazy walker and the moulted one count the same surprises while the lazy one's map stays $96\%$ false — the lie lives where one does not walk. What tells them apart is the map itself. A patrol — replaying the least recently verified transitions — turns hidden lies into signal and scrubs the map piecewise at one surprise per edge; patrol plus moult reaches the same fully honest map at a fraction of the cost, because one threshold discards all the lies at once. Self-revision closes without any external oracle: patrol detects, moult repairs wholesale, patrol re-verifies. And the practice of revision is itself *licensed by depth* — executed only when the meta-level is conscious — yet the hierarchy is fault-tolerant by construction: with the licence blocked, the lower reflex still keeps the map clean, only paying in reflexes where a glance would have sufficed. Depth is not a privilege; it is the cheap way of doing what is otherwise done dearly.
