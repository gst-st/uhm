...
**Примечание.** Каноническая форма $R = 1/(7P)$ **[Т]** — единственная через теорему Ченцова--Петца (Sol.77). Эквивалентная форма: $R = 1 - \|\Gamma - \rho^*_{\mathrm{diss}}\|_F^2 / P$, где $\rho^*_{\mathrm{diss}} = I/7$. Вывод см. в [мере рефлексии](/docs/consciousness/foundations/self-observation#мера-рефлексии-r).
:::
