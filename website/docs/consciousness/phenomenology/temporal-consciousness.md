...
Феноменологическое «настоящее» (~300 мс по данным Varela, Pöppel) может быть выведено из O-сектора. Формула субъективного времени $dt_{\text{subj}}/dt_{\text{phys}} = |\gamma_{OE}|/\gamma_{OO}$ [С] определяет временнóе окно интеграции. При типичных значениях $\gamma_{OO} \sim 1$ и $|\gamma_{OE}| \sim 0.3$ (порог осознания), характерное время: $T_{\text{present}} \sim 1/(\omega_0 \cdot \gamma_{OO}) \sim 300$ мс при $\omega_0 \sim 3$ Гц (тета-ритм). Статус: [Г]. Требуется калибровка $\omega_0$.
:::
