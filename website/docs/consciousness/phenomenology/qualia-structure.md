...
**(a) The $\varphi$-vector is carried by phases alone.** On a phase-free (real) state $A=0$, hence $v\equiv 0$ identically, while the symmetric quality persists (machine: $|v|=0,\ |A|=0,\ |S_0|=0.3446$ on a random real state; on a random complex state $|v|=0.3797$). So the seven-dimensional quality channel lives *only* in coherence phases — and therefore decays at the decoherence rate $5\gamma/21$ (T-59). This is the exact object behind the intuition of "seven-dimensional coherences being decoded": it is a genuine seven-component vector, obtained from the coherence phases by the Fano multiplication table.
