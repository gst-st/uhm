...
**(b)** Произведение опытов $F(\Gamma_1) \boxtimes F(\Gamma_2) = (s_1 \otimes s_2, q_1 \times q_2, (c_1, c_2))$ — покомпонентное произведение (спектры перемножаются, качества конкатенируются).
...
**(d)** **Необратимость.** Проекция $\mu: F(\Gamma_1 \otimes \Gamma_2) \to F(\Gamma_1) \boxtimes F(\Gamma_2)$ (частичный след) теряет информацию о корреляциях. При $\Phi_{12} > 1$ потеря строго положительна: $\Phi_{12} > 1 \Rightarrow P_{\text{coh}}^{(12)} > P_{\text{diag}}^{(12)}$ (T-129 [Т]), что невозможно воспроизвести из произведения.

**(e)** **Колакс-моноидальность.** Существование (необратимой) проекции $\mu$ делает $F$ **колакс-моноидальным функтором** (coherence map направлена F(A⊗B) → F(A)⊠F(B), но не обратно). При $\Phi_{12} < 1$ проекция обратима — $F$ локально моноидален. $\blacksquare$

**Категорная формулировка проблемы комбинации:** Проблема комбинации = вопрос «является ли $F$ строго моноидальным?». УГМ даёт точный ответ: **нет при $\Phi > 1$** [Т]. Интегрированный опыт ($\Phi_{12} > 1$) **несводим** к произведению опытов частей. Это математическая теорема о структуре функтора, не философское утверждение.
