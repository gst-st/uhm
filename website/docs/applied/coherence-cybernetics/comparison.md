...
2. **Calibration — half closed, half open.** The monitoring thresholds ($\sigma_1 \approx 0.5$, $\sigma_2 \approx 0.7$, $\sigma_3 \approx 0.9$) have a **[T]** structure and **[C]** numbers: they depend on $\lVert\bar h\rVert_{\mathrm{typical}}$ for the specific system ([T-106](./diagnostics#вывод-порогов)). What used to be listed here as fully unsolved — "translating observations into an element of $\Gamma$" — is **no longer** unsolved: given a 7-channel embedding, $\Gamma$ is the normalised second moment, and its recovery is a *provably consistent* estimator with matrix-Bernstein confidence bounds, a debiased purity estimator, and an explicit sample complexity for a threshold call ([rigorous $\Gamma$-estimator](./measurement#оценка-gamma) [T], with a [reference implementation](pathname:///code/gamma_tomography.py)). What genuinely **remains** open is narrower and cleanly isolated: the choice of the embedding $\pi$ itself — the semantic bridge from a specific substrate to the dimensions $(A,\dots,U)$ — and the empirical question of whether one $\pi$ transfers across substrates.
