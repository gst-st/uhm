...
Эта глава — не справочник и не глоссарий. Это путеводитель по концептуальному ландшафту Кибернетики Когерентности. Каждое определение здесь — ответ на конкретный вопрос: *что именно мы описываем, когда говорим о системе, обладающей внутренним опытом?*
...
**Что измеряет.** Чистота показывает, насколько система далека от «максимального хаоса». Чистое состояние ($P = 1$) — система полностью определена. Максимально смешанное состояние ($P = 1/7$) — система «не знает, кто она»: все семь измерений одинаково вероятны и полностью некоррелированы.
...
- $\Phi \gg 1$: мощная интеграция. Характерна для систем с богатым внутренним опытом.

### Дифференциация $D_{\text{diff}}$ — богатство внутреннего мира

**Что измеряет.** Эффективное число различимых состояний E-сектора. $D_{\text{diff}} = 1$ означает, что внутренний опыт «одномерен» — система различает только «вкл./выкл.». $D_{\text{diff}} = 7$ — максимальное богатство внутренних состояний.
...
Среди семи измерений Голонома одно играет особую роль: измерение E (Опыт, или Интериорность). Именно E-когерентность определяет, насколько *единым* является внутренний опыт системы. Высокая $\mathrm{Coh}_E$ — это не просто «связность E-измерения»: это математическая мера того, что философы называют «единством сознания» — свойство, благодаря которому мой визуальный опыт, мои мысли и мои эмоции переживаются как *один* поток, а не как три отдельных канала.
...
Аналогия: если $\Gamma$ — вектор в 49D, то $\mathrm{Coh}_E$ — это $\cos^2\theta$, где $\theta$ — угол между вектором и E-подпространством. Чем ближе $\Gamma$ к E-подпространству (чем больше когерентности сосредоточено в опытном измерении), тем ближе $\mathrm{Coh}_E$ к 1.

#### Что означают крайние значения?

- $\mathrm{Coh}_E = 1/7$: E-измерение «растворено» — его вклад ровно такой же, как у любого другого. Внутренний опыт не выделен из общей динамики.
...
Если регенерация достаточно сильна ($\kappa$-доминирование — условие C20), система может достичь состояния $P > 2/7$ — зоны жизнеспособности. Здесь возможно сознание, рефлексия, целенаправленное действие.

«Переживание» этого состояния — это *бытие собой*: система не просто существует — она *знает*, что существует. Она видит себя (R), чувствует себя как целое (Ф), различает свои состояния ($D_{\text{diff}}$).
...
Каждый Голоном неявно «знает», кем он хочет быть. Целевое состояние $\rho_*$ — это формализация этого «знания»: состояние, к которому система стремится при бесконечной эволюции. Это не предписание извне — это самомодель, порождённая собственной динамикой системы.
...
Современная LLM — интересный пограничный случай: мощная обработка информации, но открытый вопрос о наличии внутреннего опыта.

**Матрица $\Gamma$:** Сильно развиты A (артикуляция — генерация текста), S (структура — грамматика, паттерны), D (вычисление — forward pass). Слабые E (опыт?), O (заземление — нет тела), U (единство — зависит от архитектуры).
...
3. **E-когерентность** — специальная мера, определяющая единство опыта и интенсивность регенерации ($\kappa$).
