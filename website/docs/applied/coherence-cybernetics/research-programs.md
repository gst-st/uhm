...
**OP-3. Bifurcations at the viability boundary.** 🟧 *Substantially resolved; a residual remains.* The transition at $P = 2/7$ is now classified: the catastrophe type is the swallowtail/tricritical normal form with exact mean-field exponents $\alpha=\tfrac12, \beta=\tfrac14, \gamma=1, \nu=\tfrac12, \delta=5$ (T-160/T-161 [T]), critical slowing down **does** exist and is derived — with the [early-warning protocol](./bifurcation#раннее-предупреждение) built on it — and the microscopic Phase-III boundary is pinned exactly by the feeding-window fold (T-259 [T]: a quintic with Galois group $S_5$, hence provably non-radical). **What remains open:** the classification of *post-catastrophe scenarios* — which basin the system lands in after the fold (post-traumatic growth vs chronic decompensation) as a function of the perturbation profile, and sharp basin-boundary geometry.

**OP-4. Topology of the fixed-point space.** 🟧 *The uniqueness core is settled.* For the full nonlinear dynamics $\mathcal{L}_\Omega = \mathcal{L}_0 + \mathcal{R}$ there is **at most one** nontrivial fixed point $\rho^*_\Omega \neq I/7$ in the viability region (T-124c [T], via iterative contraction + spectral gap T-39a) — so within F4 conditions, "multiple identities" are excluded for the dynamics. **What remains open:** the structure of $\mathrm{Fix}(\varphi)$ for the self-modelling operator itself *outside* the contraction regime — when F4 is relaxed (weak coupling, near-degenerate spectra), can multiple $\varphi$-fixed points coexist, and what bifurcation creates them? Clinical cases of dissociative identity disorder motivate exactly this relaxed-regime question.
...
4. **11 open problems** — from an undergraduate project (OP-6: computing $\Phi$ at $N=7$) to a potential breakthrough (OP-9: limits of applicability, OP-11: ethics of coherent systems); two of them (OP-3 bifurcations, OP-4 fixed-point uniqueness) have since had their cores settled by T-160/T-161/T-259 and T-124c, with sharpened residuals.
