...
Status honesty: the signature theorem is [Т]; the dictionary itself — the identification with (work, heat, matter) and with SLU's $(A, T, \mu)$ — is an interpretation [И], exact on signatures and counting. On the *dynamical laws* the ledger now reads: the matter channel is **derived** — regeneration is exactly the BKM natural-gradient descent of the free energy $D(\rho_*\|\Gamma)$ ([T-261](/docs/core/dynamics/evolution#теорема-регенерация-градиентный-спуск)), the quantum-information instance of SLU's covariant gradient descent; the compactness of the gauge torus is derived ([T-260](/docs/core/operators/lindblad-operators#теорема-происхождение-тора)); the work and heat legs remain structural [И].
