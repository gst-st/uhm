...
:::info Предсказание [Г] — структурное
$N = 7$ — минимальное $N$ для социального обучения ($K \geq 2$ агентов). Социальное обучение требует одновременно:
- **ToM** ($\varphi$-оператор): $\geq 3$ когнитивных измерения (триадная декомпозиция, T-57)
- **Communication** (ISL): $\geq 3$ когнитивных измерения (Фано-грамматика, T-114)
...
## Предсказание 12: Потолок глубины самоосознания (C26) {#предсказание-12}

:::info Предсказание [С] + числовая верификация — фальсифицируемое
$$
\mathrm{SAD}_\text{max} = 3 \quad (\text{при Fano contraction } \alpha = 2/3)
$$
Ни одна конечная система ($P \leq 1$) не может достичь SAD $\geq$ 4. Это следствие формулы $P_{\text{crit}}^{(n)} = P_{\text{crit}} \cdot 3^{n-1}/(n+1)$ ([C26](/docs/consciousness/hierarchy/depth-tower#критическая-чистота-sad)).
...
**Верифицируемость:** Для воплощённого агента с backbone injection, измерить аттрактор $P^*$. Если $P^* \ll 3/7$ или $P^* > 3/7$ — предсказание 15 фальсифицировано.
...
**Верифицируемость:** Если реализация SYNARC показывает $\|\pi - \pi_{\mathrm{can}}\|_\diamond > \varepsilon_0$ при обученном $\pi$ — архитектура требует пересмотра. Нарушение T-152 означает потерю CPTP-свойства в процессе обучения.
...
| 12 | [Потолок SAD](#предсказание-12) | $\mathrm{SAD}_\text{max} = 3$ | **[С]** | Отсутствует |
