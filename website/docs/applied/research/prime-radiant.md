...
What is *not* demonstrable today, stated as sharply: any claim about an external system the machine has not encoded — the estimator organ [K] now exists and is validated on machine-generated observation models with known ground truth (demo 6); what remains open — and is said so — is the *domain sensor*: the encoding that turns an external system's raw signals into the observation stream the filter consumes.
...
2. **The Estimator** (Kalman, Delphi) — infer the state from noisy data, repeatedly. *Machine: [K], shipped in v0.6 — validated on machine-generated observation models (demo 6); the domain sensor for external systems remains open.*
...
3. ~~The Estimator organ [K]~~ — **shipped in v0.6** (`estimator.rs`, demo 6, R26–R27): predict–measure–update over $\mathcal{D}(\mathbb{C}^7)$ with process noise, full and magnitude-only partial readouts; beats both baselines; end-to-end diagnosis through noise 62% vs 22% raw. Remaining: the *domain sensor* (external signals → observation stream) and multi-encoder Delphi consensus.
