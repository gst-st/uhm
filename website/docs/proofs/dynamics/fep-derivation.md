...
**Лемма 4.1 (Эквивалентность условий).**

Для 7-мерного голонома, условия (a) и (b) связаны:

Чистота $P = \mathrm{Tr}(\Gamma^2) = \sum_i p_i^2 + 2\sum_{i < j} |\Gamma_{ij}|^2$. При $\Gamma_{ij} \to 0$ чистота определяется только диагональными элементами: $P = \sum_i p_i^2$. Рефлексия $R = 1/(7P)$ минимальна при $P = 1$ (одно собственное значение равно 1), что соответствует полностью «классической» определённости.

*Доказательство.* Внедиагональные когерентности $|\Gamma_{ij}|$ непосредственно входят в Gap-оператор $\mathcal{G}_{ij}$ (см. [динамика Gap](/docs/core/dynamics/gap-dynamics)). При $\Gamma_{ij} \to 0$ все Gap-элементы обнуляются: $\mathcal{G}_{ij} \to 0$. Одновременно, из определения $R = 1/(7P)$ следует, что $R$ монотонно убывает с ростом $P$, достигая $R_{\min} = 1/7$ при $P = 1$. Обратно: если $R \to 1/7$, то $P \to 1$, и матрица плотности стремится к проектору ранга 1, что при N-мерном ограничении и нормировке допускает только диагональные состояния с одним доминирующим собственным значением. $\blacksquare$
...
$$
\mathcal{F}[\psi; \Gamma] = S_{vN}(\psi(\Gamma)) + D_{KL}(\psi(\Gamma) \| \Gamma) \geq S_{vN}(\psi(\Gamma)) \geq S_{vN}(\Gamma_*)
$$

где последнее неравенство следует из того, что стационарное состояние $\Gamma_*$ минимизирует энтропию на множестве достижимых состояний. В классическом пределе $S_{vN}(\Gamma) = H(p) = -\sum_i p_i \log p_i$, и неравенство принимает форму $F \geq -\ln p(o)$, если отождествить $-\ln p(o)$ с энтропией маргинальной вероятности наблюдений. $\blacksquare$
